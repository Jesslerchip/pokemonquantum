static const struct TrainerMon sParty_Logan[] = {
    {
    .nickname = _("Zorro"),
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZORUA,
    .moves = {MOVE_SCRATCH, MOVE_LEER, MOVE_TORMENT, MOVE_NONE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_SASSY,
    .ball = ITEM_DUSK_BALL,
    }
};

static const struct TrainerMon sParty_Mike[] = {
    {
    .nickname = _("Pat"),
    .iv = 20,
    .lvl = 3,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_PATRAT,
    .moves = {MOVE_TACKLE, MOVE_LEER, MOVE_NONE, MOVE_NONE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_TIMID,
    .ball = ITEM_REPEAT_BALL,
    },
    {
    .nickname = _("Nicky"),
    .iv = 20,
    .lvl = 3,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NICKIT,
    .moves = {MOVE_BITE, MOVE_SCARY_FACE, MOVE_NONE, MOVE_NONE},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_NAIVE,
    .ball = ITEM_POKE_BALL,
    }
};

static const struct TrainerMon sParty_John[] = {
    {
    .nickname = _("Sunshrew"),
    .iv = 0,
    .lvl = 3,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_SANDSHREW,
    .moves = {MOVE_SCRATCH, MOVE_DEFENSE_CURL, MOVE_POISON_STING, MOVE_NONE},
    .nature = NATURE_QUIET,
    .ball = ITEM_GREAT_BALL,
    .shiny = TRUE,
    },
    {
    .nickname = _("Snowshrew"),
    .iv = 0,
    .lvl = 3,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_SANDSHREW_ALOLAN,
    .moves = {MOVE_SCRATCH, MOVE_DEFENSE_CURL, MOVE_MIST, MOVE_NONE},
    .nature = NATURE_SASSY,
    .ball = ITEM_TIMER_BALL,
    }
};

static const struct TrainerMon sParty_Paul[] = {
    {
    .nickname = _("Bunbo"),
    .iv = 0,
    .lvl = 3,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BUNNELBY,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_MUD_SLAP, MOVE_LEER, MOVE_TACKLE, MOVE_NONE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_SASSY,
    .ball = ITEM_POKE_BALL,
    },
    {
    .nickname = _("Psydkick"),
    .iv = 0,
    .lvl = 3,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PSYDUCK,
    .moves = {MOVE_SCRATCH, MOVE_TAIL_WHIP, MOVE_WATER_GUN, MOVE_NONE},
    .nature = NATURE_JOLLY,
    .ball = ITEM_POKE_BALL,
    },
        {
    .nickname = _("Picnickit"),
    .iv = 0,
    .lvl = 3,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NICKIT,
    .moves = {MOVE_QUICK_ATTACK, MOVE_TAIL_WHIP, MOVE_BEAT_UP, MOVE_NONE},
    .nature = NATURE_LAX,
    .ball = ITEM_LUXURY_BALL,
    .shiny = TRUE,
    }
};

static const struct TrainerMon sParty_Timmy[] = {
   {
    .nickname = _("Squirmpy"),
    .iv = 0,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WURMPLE,
    .moves = {MOVE_TACKLE, MOVE_STRING_SHOT, MOVE_NONE, MOVE_NONE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_BRAVE,
    .ball = ITEM_POKE_BALL,
    },
    {
    .nickname = _("Flurmpy"),
    .iv = 0,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WURMPLE,
    .moves = {MOVE_TACKLE, MOVE_STRING_SHOT, MOVE_NONE, MOVE_NONE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_LONELY,
    .ball = ITEM_POKE_BALL,
    },
    {
    .nickname = _("Purmpy"),
    .iv = 0,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WURMPLE,
    .moves = {MOVE_TACKLE, MOVE_STRING_SHOT, MOVE_NONE, MOVE_NONE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_POKE_BALL,
    },
    {
    .nickname = _("Churmpy"),
    .iv = 0,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WURMPLE,
    .moves = {MOVE_TACKLE, MOVE_STRING_SHOT, MOVE_NONE, MOVE_NONE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_NAUGHTY,
    .ball = ITEM_POKE_BALL,
    },
};

static const struct TrainerMon sParty_Poppy[] = {
    {
    .nickname = _("Purrty"),
    .iv = 0,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PURRLOIN,
    .moves = {MOVE_SCRATCH, MOVE_GROWL, MOVE_SAND_ATTACK, MOVE_NONE},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_HASTY,
    .ball = ITEM_HEAL_BALL,
    },
    {
    .nickname = _("Shinshin"),
    .iv = 0,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_SHINX,
    .moves = {MOVE_TACKLE, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_NONE},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_NAIVE,
    .ball = ITEM_HEAL_BALL,
    .shiny = TRUE,
    },
};

static const struct TrainerMon sParty_Richtor[] = {
    {
    .nickname = _("Sebastian"),
    .iv = 20,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VULPIX,
    .moves = {MOVE_EMBER, MOVE_TAIL_WHIP, MOVE_DISABLE, MOVE_NONE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_GREAT_BALL,
    },
        {
    .nickname = _("Eric"),
    .iv = 0,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZORUA,
    .moves = {MOVE_SCRATCH, MOVE_LEER, MOVE_TORMENT, MOVE_NONE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname = _("Savannah"),
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VULPIX_ALOLAN,
    .moves = {MOVE_POWDER_SNOW, MOVE_TAIL_WHIP, MOVE_DISABLE, MOVE_NONE},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_GREAT_BALL,
    },
};

static const struct TrainerMon sParty_Bella1[] = {
    {
    .nickname = _("Tempest"),
    .iv = 20,
    .lvl = 7,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_SHINX,
    .moves = {MOVE_TACKLE, MOVE_LEER, MOVE_THUNDER_SHOCK, MOVE_NONE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_QUICK_BALL,
    .shiny = TRUE,
    },
        {
    .nickname = _("Indominus"),
    .iv = 20,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TYRUNT,
    .moves = {MOVE_TACKLE, MOVE_TAIL_WHIP, MOVE_ROAR, MOVE_ANCIENT_POWER},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Arrow"),
    .iv = 20,
    .lvl = 7,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_FLETCHLING,
    .moves = {MOVE_PECK, MOVE_GROWL, MOVE_QUICK_ATTACK, MOVE_NONE},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_PREMIER_BALL,
    .shiny = TRUE,
    },
    {
    .nickname = _("Pamper"),
    .iv = 20,
    .lvl = 9,
    .evs = {0, 20, 0, 0, 0, 20},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_SKITTY,
    .moves = {MOVE_FAKE_OUT, MOVE_TACKLE, MOVE_TAIL_WHIP, MOVE_SING},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_LUXURY_BALL,
    .shiny = TRUE,
    },
};

static const struct TrainerMon sParty_Vilder[] = {
    {
    .nickname = _("Prophet"),
    .iv = 100,
    .lvl = 9,
    .evs = {20, 0, 20, 4, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_NATU,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_STORED_POWER, MOVE_NONE, MOVE_LEER, MOVE_CALM_MIND},
    .nature = NATURE_HASTY,
    .ball = ITEM_TIMER_BALL,
    },
    {
    .nickname = _("Reaper"),
    .iv = 200,
    .lvl = 10,
    .evs = {0, 20, 0, 0, 4, 20},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_MURKROW,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_NONE, MOVE_WING_ATTACK, MOVE_CONFUSE_RAY, MOVE_ROOST},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DUSK_BALL,
    }
};

static const struct TrainerMon sParty_GruntPetalburgWoods[] = {
    {
    .nickname = _("Magicae"),
    .iv = 255,
    .lvl = 25,
    .evs = {0, 0, 4, 252, 0, 252},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_BRAIXEN,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_PSYBEAM, MOVE_FIRE_SPIN, MOVE_WILL_O_WISP, MOVE_WISH},
    .nature = NATURE_MODEST,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Luna"),
    .iv = 255,
    .lvl = 26,
    .evs = {128, 0, 0, 252, 36, 92},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_MUNNA,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_FUTURE_SIGHT, MOVE_MOONLIGHT, MOVE_DAZZLING_GLEAM, MOVE_HYPNOSIS},
    .nature = NATURE_MODEST,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Ignis"),
    .iv = 255,
    .lvl = 27,
    .evs = {0, 4, 0, 252, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VICTINI,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_NONE, MOVE_PSYCHIC, MOVE_BOLT_STRIKE, MOVE_ROCK_SLIDE},
    .nature = NATURE_MODEST,
    .ball = ITEM_DUSK_BALL,
    }
};

static const struct TrainerMon sParty_Marcel[] = {
    {
    .nickname = _("Bulby"),
    .iv = 100,
    .lvl = 7,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BULBASAUR,
    .moves = {MOVE_LEECH_SEED, MOVE_VINE_WHIP, MOVE_GROWL, MOVE_PROTECT},
    .nature = NATURE_SASSY,
    .ball = ITEM_NEST_BALL,
    },
    {
    .nickname = _("Charbo"),
    .iv = 100,
    .lvl = 7,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CHARMANDER,
    .moves = {MOVE_EMBER, MOVE_COUNTER, MOVE_SWAGGER, MOVE_BLOCK},
    .nature = NATURE_MILD,
    .ball = ITEM_FAST_BALL,
    },
    {
    .nickname = _("Squirp"),
    .iv = 100,
    .lvl = 7,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SQUIRTLE,
    .moves = {MOVE_WATER_GUN, MOVE_WITHDRAW, MOVE_ICE_PUNCH, MOVE_FLAIL},
    .nature = NATURE_RELAXED,
    .ball = ITEM_DIVE_BALL,
    }
};

static const struct TrainerMon sParty_Alberto[] = {
    {
    .nickname = _("Heliopkyle"),
    .iv = 0,
    .lvl = 9,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_HELIOPTILE,
    .moves = {MOVE_THUNDER_WAVE, MOVE_THUNDER_SHOCK, MOVE_U_TURN, MOVE_LOW_SWEEP},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_NAIVE,
    .ball = ITEM_SAFARI_BALL,
    },
    {
    .nickname = _("Slogan"),
    .iv = 0,
    .lvl = 9,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_SLOWPOKE,
    .moves = {MOVE_STORED_POWER, MOVE_CURSE, MOVE_ICY_WIND, MOVE_WEATHER_BALL},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_SASSY,

    .ball = ITEM_DIVE_BALL,
    }
};

static const struct TrainerMon sParty_Ed[] = {
    {
    .nickname = _("Barb"),
    .iv = 20,
    .lvl = 9,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_BARBOACH,
    .moves = {MOVE_WATER_GUN, MOVE_MUD_SHOT, MOVE_ICY_WIND, MOVE_SANDSTORM},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_HASTY,
    .ball = ITEM_DIVE_BALL,
    },
    {
    .nickname = _("Zapra"),
    .iv = 20,
    .lvl = 10,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BLITZLE,
    .moves = {MOVE_QUICK_ATTACK, MOVE_CHARGE, MOVE_CHARGE_BEAM, MOVE_PROTECT},
    .nature = NATURE_NAIVE,
    .ball = ITEM_REPEAT_BALL,
    }
};

static const struct TrainerMon sParty_GruntSeafloorCavern4[] = {
    {
    .iv = 0,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_Declan[] = {
    {
    .nickname = _("Tikky"),
    .iv = 20,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_JOLTIK,
    .moves = {MOVE_THUNDER_WAVE, MOVE_BUG_BITE, MOVE_VOLT_SWITCH, MOVE_SCREECH},
    .nature = NATURE_IMPISH,
    .ball = ITEM_NET_BALL,
    },
    {
    .nickname = _("Venbo"),
    .iv = 20,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VENIPEDE,
    .moves = {MOVE_ROLLOUT, MOVE_POISON_STING, MOVE_TOXIC_SPIKES, MOVE_HEX},
    .nature = NATURE_LONELY,
    .ball = ITEM_NET_BALL,
    },
    {
    .nickname = _("Dairyfree"),
    .iv = 20,
    .lvl = 10,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_BUTTERFREE,
    .moves = {MOVE_CONFUSION, MOVE_GUST, MOVE_SAFEGUARD, MOVE_SUPERSONIC},
    .nature = NATURE_GENTLE,
    .ball = ITEM_NET_BALL,
    },
    {
    .nickname = _("King"),
    .iv = 20,
    .lvl = 10,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSTOX,
    .moves = {MOVE_SUBSTITUTE, MOVE_TOXIC, MOVE_ROOST, MOVE_GUST},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_RELAXED,
    .ball = ITEM_NET_BALL,
    },
    {
    .nickname = _("Polka"),
    .iv = 20,
    .lvl = 10,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DOTTLER,
    .moves = {MOVE_CONFUSION, MOVE_TRICK_ROOM, MOVE_CALM_MIND, MOVE_LIGHT_SCREEN},
    .nature = NATURE_QUIET,
    .ball = ITEM_NET_BALL,
    },
    {
    .nickname = _("Fry"),
    .iv = 20,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SIZZLIPEDE,
    .moves = {MOVE_EMBER, MOVE_SMOKESCREEN, MOVE_BRUTAL_SWING, MOVE_VENOSHOCK},
    .nature = NATURE_MILD,
    .ball = ITEM_NET_BALL,
    }
};

static const struct TrainerMon sParty_GruntRusturfTunnel[] = {
{
    .nickname = _("Photon"),
    .iv = 255,
    .lvl = 36,
    .evs = {0, 0, 0, 252, 4, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROTOM,
    .heldItem = ITEM_LUM_BERRY,
    .moves = {MOVE_DEFOG, MOVE_THUNDER, MOVE_SHADOW_BALL, MOVE_VOLT_SWITCH},
    .nature = NATURE_MODEST,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Quark"),
    .iv = 255,
    .lvl = 37,
    .evs = {0, 0, 0, 252, 4, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_ELECTRODE,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_THUNDER_WAVE, MOVE_ROLLOUT, MOVE_EXPLOSION, MOVE_RAPID_SPIN},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Entropy"),
    .iv = 255,
    .lvl = 37,
    .evs = {0, 0, 0, 252, 4, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_STARMIE,
    .heldItem = ITEM_ROCKY_HELMET,
    .moves = {MOVE_HYDRO_PUMP, MOVE_ICE_BEAM, MOVE_PSYSHOCK, MOVE_RAPID_SPIN},
    .nature = NATURE_MODEST,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Inertia"),
    .iv = 255,
    .lvl = 37,
    .evs = {252, 0, 252, 0, 4, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PORYGON2,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_RECOVER, MOVE_ICE_BEAM, MOVE_FOUL_PLAY, MOVE_TOXIC},
    .nature = NATURE_MODEST,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Cathode"),
    .iv = 255,
    .lvl = 39,
    .evs = {4, 0, 0, 252, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_AMPHAROSITE,
    .moves = {MOVE_VOLT_SWITCH, MOVE_DRAGON_PULSE, MOVE_FOCUS_BLAST, MOVE_THUNDERBOLT},
    .nature = NATURE_MODEST,
    .ball = ITEM_MASTER_BALL,
    .shiny = TRUE,
    }
};

static const struct TrainerMon sParty_GruntWeatherInst1[] = {
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    },
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    }
};

static const struct TrainerMon sParty_GruntWeatherInst2[] = {
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_GruntWeatherInst3[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_GruntMuseum1[] = {
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_GruntMuseum2[] = {
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    },
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter1[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_GruntMtPyre1[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_GruntMtPyre2[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_GruntMtPyre3[] = {
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_GruntWeatherInst4[] = {
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_GruntAquaHideout5[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_GruntAquaHideout6[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_Fredrick[] = {
    {
    .nickname = _("Bye"),
    .iv = 100,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_GOSSIFLEUR,
    .moves = {MOVE_BULLET_SEED, MOVE_RAPID_SPIN, MOVE_HELPING_HAND, MOVE_GRASSY_TERRAIN},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_LOVE_BALL,
    },
    {
    .nickname = _("Pann"),
    .iv = 100,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_PETILIL,
    .moves = {MOVE_MEGA_DRAIN, MOVE_STUN_SPORE, MOVE_HELPING_HAND, MOVE_PROTECT},
    .nature = NATURE_MODEST,
    .ball = ITEM_LOVE_BALL,
    }
};

static const struct TrainerMon sParty_Matt[] = {
    {
    .nickname = _("Yeti"),
    .iv = 100,
    .lvl = 12,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DARUMAKA_GALARIAN,
    .moves = {MOVE_AVALANCHE, MOVE_BITE, MOVE_BRICK_BREAK, MOVE_ROCK_TOMB},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_PREMIER_BALL,
    },
    {
    .nickname = _("1-Up"),
    .iv = 100,
    .lvl = 13,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_MORELULL,
    .moves = {MOVE_CONFUSE_RAY, MOVE_INGRAIN, MOVE_MEGA_DRAIN, MOVE_PROTECT},
    .nature = NATURE_MODEST,
    .ball = ITEM_SAFARI_BALL,
    }
};

static const struct TrainerMon sParty_Zander[] = {
    {
    .nickname = _("Olympus"),
    .iv = 50,
    .lvl = 13,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_CLOBBOPUS,
    .moves = {MOVE_ROCK_SMASH, MOVE_FEINT, MOVE_ICE_PUNCH, MOVE_PAYBACK},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname = _("Ostrido"),
    .iv = 50,
    .lvl = 13,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_DODUO,
    .moves = {MOVE_DOUBLE_KICK, MOVE_PECK, MOVE_QUICK_ATTACK, MOVE_DOUBLE_TEAM},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_QUICK_BALL,
    }
};

static const struct TrainerMon sParty_ShellyWeatherInstitute[] = {
    {
    .iv = 50,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    },
    {
    .iv = 50,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMon sParty_ShellySeafloorCavern[] = {
    {
    .iv = 100,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    },
    {
    .iv = 100,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMon sParty_Archie[] = {
    {
    .iv = 150,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 150,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CROBAT,
    },
    {
    .iv = 150,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Leah[] = {
    {
    .nickname = _("Icepick"),
    .iv = 100,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_SNEASEL,
    .heldItem = ITEM_ROCKY_HELMET,
    .moves = {MOVE_METAL_CLAW, MOVE_ICE_PUNCH, MOVE_FLING, MOVE_WHIRLPOOL},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_TIMER_BALL,
    },
    {
    .nickname = _("Riptide"),
    .iv = 100,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_BUIZEL,
    .moves = {MOVE_DIVE, MOVE_HAIL, MOVE_DIG, MOVE_AQUA_RING},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DIVE_BALL,
    },
};

static const struct TrainerMon sParty_Daisy[] = {
    {
    .nickname = _("Setup"),
    .iv = 100,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_LEECH_SEED, MOVE_TOXIC_SPIKES, MOVE_MEGA_DRAIN, MOVE_LIFE_DEW},
    .nature = NATURE_MODEST,
    .ball = ITEM_PARK_BALL,
    },
    {
    .nickname = _("Initialize"),
    .iv = 100,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_GIRAFARIG,
    .moves = {MOVE_BULLDOZE, MOVE_CONFUSION, MOVE_TOXIC, MOVE_SUBSTITUTE},
    .nature = NATURE_NAIVE,
    .ball = ITEM_SAFARI_BALL,
    },
    {
    .nickname = _("Sweep"),
    .iv = 100,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALCREMIE,
    .moves = {MOVE_DRAINING_KISS, MOVE_SWEET_KISS, MOVE_MYSTICAL_FIRE, MOVE_PSYSHOCK},
    .nature = NATURE_MODEST,
    .ball = ITEM_LOVE_BALL,
    },
};

static const struct TrainerMon sParty_Rose1[] = {
    {
    .nickname = _("Polarity"),
    .iv = 100,
    .lvl = 18,
    .evs = {0, 0, 0, 252, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_MAGNEMITE,
    .moves = {MOVE_VOLT_SWITCH, MOVE_GYRO_BALL, MOVE_PROTECT, MOVE_THUNDER_WAVE},
    .nature = NATURE_BOLD,
    .ball = ITEM_POKE_BALL,
    },
    {
    .nickname = _("Electra"),
    .iv = 100,
    .lvl = 18,
    .evs = {0, 0, 0, 252, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELEKID,
    .moves = {MOVE_SHOCK_WAVE, MOVE_ICE_PUNCH, MOVE_BRICK_BREAK, MOVE_MUD_SLAP},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_NAIVE,
    .ball = ITEM_QUICK_BALL,
    },
    {
    .nickname = _("Beacon"),
    .iv = 100,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_JOLTEON,
    .moves = {MOVE_THUNDERBOLT, MOVE_SING, MOVE_DIG, MOVE_SHADOW_BALL},
    .nature = NATURE_NAIVE,
    .ball = ITEM_NEST_BALL,
    }
};

static const struct TrainerMon sParty_Felix[] = {
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_PSYCHIC, MOVE_NONE, MOVE_NONE, MOVE_NONE}
    },
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CLAYDOL,
    .moves = {MOVE_SKILL_SWAP, MOVE_EARTHQUAKE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Violet[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLOOM,
    }
};

static const struct TrainerMon sParty_Rose2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 10,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Rose3[] = {
    {
    .iv = 20,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 20,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 20,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Rose4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Rose5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Dusty1[] = {
    {
    .nickname = _("Cutie"),
    .iv = 20,
    .lvl = 6,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_SENTRET,
    .moves = {MOVE_TRICK, MOVE_DEFENSE_CURL, MOVE_COVET, MOVE_BABY_DOLL_EYES},
    .nature = NATURE_RELAXED,
    .ball = ITEM_HEAL_BALL,
    },
    {
    .nickname = _("Boop"),
    .iv = 20,
    .lvl = 6,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_PIDGEY,
    .moves = {MOVE_PLUCK, MOVE_SAND_ATTACK, MOVE_DOUBLE_TEAM, MOVE_ROOST},
    .nature = NATURE_JOLLY,
    .ball = ITEM_HEAL_BALL,
    }
};

static const struct TrainerMon sParty_Chip[] = {
    {
    .iv = 50,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BALTOY,
    .moves = {MOVE_PSYBEAM, MOVE_SELF_DESTRUCT, MOVE_SANDSTORM, MOVE_ANCIENT_POWER}
    },
    {
    .iv = 50,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    .moves = {MOVE_DIG, MOVE_SLASH, MOVE_SAND_ATTACK, MOVE_POISON_STING}
    },
    {
    .iv = 50,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_DIG, MOVE_SLASH, MOVE_SAND_ATTACK, MOVE_POISON_STING}
    }
};

static const struct TrainerMon sParty_Foster[] = {
    {
    .iv = 100,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    .moves = {MOVE_DIG, MOVE_SLASH, MOVE_SAND_ATTACK, MOVE_POISON_STING}
    },
    {
    .iv = 100,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_DIG, MOVE_SLASH, MOVE_SAND_ATTACK, MOVE_POISON_STING}
    }
};

static const struct TrainerMon sParty_Dusty2[] = {
    {
    .iv = 60,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_DIG, MOVE_SLASH, MOVE_SAND_ATTACK, MOVE_POISON_STING}
    }
};

static const struct TrainerMon sParty_Dusty3[] = {
    {
    .iv = 70,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_DIG, MOVE_SLASH, MOVE_SAND_ATTACK, MOVE_POISON_STING}
    }
};

static const struct TrainerMon sParty_Dusty4[] = {
    {
    .iv = 80,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_DIG, MOVE_SLASH, MOVE_SAND_ATTACK, MOVE_POISON_STING}
    }
};

static const struct TrainerMon sParty_Dusty5[] = {
    {
    .iv = 90,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSLASH,
    .moves = {MOVE_DIG, MOVE_SLASH, MOVE_SAND_ATTACK, MOVE_POISON_STING}
    }
};

static const struct TrainerMon sParty_GabbyAndTy1[] = {
    {
    .iv = 50,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 50,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WHISMUR,
    }
};

static const struct TrainerMon sParty_GabbyAndTy2[] = {
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_GabbyAndTy3[] = {
    {
    .iv = 150,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 150,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_GabbyAndTy4[] = {
    {
    .iv = 200,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 200,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_GabbyAndTy5[] = {
    {
    .iv = 250,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 250,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_GabbyAndTy6[] = {
    {
    .iv = 250,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SONIC_BOOM, MOVE_THUNDER_WAVE, MOVE_METAL_SOUND, MOVE_THUNDERBOLT}
    },
    {
    .iv = 250,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_EXPLOUD,
    .moves = {MOVE_ASTONISH, MOVE_STOMP, MOVE_SUPERSONIC, MOVE_HYPER_VOICE}
    }
};

static const struct TrainerMon sParty_Lola1[] = {
    {
    .nickname = _("EMP"),
    .iv = 255,
    .lvl = 35,
    .evs = {0, 0, 0, 252, 4, 252},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_THUNDERBOLT, MOVE_FLASH_CANNON, MOVE_TRI_ATTACK, MOVE_SCREECH},
    .nature = NATURE_MODEST,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Firebomb"),
    .iv = 255,
    .lvl = 35,
    .evs = {8, 200, 0, 200, 0, 100},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_TALONFLAME,
    .moves = {MOVE_BRAVE_BIRD, MOVE_ROOST, MOVE_WILL_O_WISP, MOVE_DEFOG},
    .nature = NATURE_JOLLY,
    .ball = ITEM_DUSK_BALL,
    }
};

static const struct TrainerMon sParty_Austina[] = {
    {
    .nickname = _("Picasso"),
    .iv = 255,
    .lvl = 35,
    .evs = {252, 0, 4, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SMEARGLE,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_STICKY_WEB, MOVE_NUZZLE, MOVE_TAUNT, MOVE_TOXIC_THREAD},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Pneumonia"),
    .iv = 255,
    .lvl = 35,
    .evs = {252, 0, 152, 0, 88, 16},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WEEZING,
    .heldItem = ITEM_ROCKY_HELMET,
    .moves = {MOVE_SLUDGE_BOMB, MOVE_WILL_O_WISP, MOVE_PAIN_SPLIT, MOVE_TOXIC_SPIKES},
    .nature = NATURE_BOLD,
    .ball = ITEM_DUSK_BALL,
    .shiny = TRUE,
    }
};

static const struct TrainerMon sParty_Gwen[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Lola2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 10,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Lola3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Lola4[] = {
    {
    .iv = 30,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 30,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Lola5[] = {
    {
    .iv = 40,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 40,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMon sParty_Ricky1[] = {
    {
    .nickname = _("Tongue"),
    .iv = 255,
    .lvl = 34,
    .evs = {248, 0, 8, 0, 252, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_LICKILICKY,
    .moves = {MOVE_WISH, MOVE_PROTECT, MOVE_TOXIC, MOVE_SEISMIC_TOSS},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_LUXURY_BALL,
    },
    {
    .nickname = _("Twister"),
    .iv = 255,
    .lvl = 34,
    .evs = {148, 248, 8, 0, 88, 16},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_BELLY_DRUM, MOVE_EXTREME_SPEED, MOVE_STOMPING_TANTRUM, MOVE_SEED_BOMB},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_LUXURY_BALL,
    }
};

static const struct TrainerMon sParty_Simon[] = {
    {
    .iv = 0,
    .lvl = 12,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AZURILL,
    },
    {
    .iv = 0,
    .lvl = 12,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Charlie[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Ricky2[] = {
    {
    .iv = 10,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_SAND_ATTACK, MOVE_PIN_MISSILE, MOVE_TAIL_WHIP, MOVE_SURF}
    }
};

static const struct TrainerMon sParty_Ricky3[] = {
    {
    .iv = 20,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_SAND_ATTACK, MOVE_PIN_MISSILE, MOVE_TAIL_WHIP, MOVE_SURF}
    }
};

static const struct TrainerMon sParty_Ricky4[] = {
    {
    .iv = 30,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_SAND_ATTACK, MOVE_PIN_MISSILE, MOVE_TAIL_WHIP, MOVE_SURF}
    }
};

static const struct TrainerMon sParty_Ricky5[] = {
    {
    .iv = 40,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_SAND_ATTACK, MOVE_PIN_MISSILE, MOVE_TAIL_WHIP, MOVE_SURF}
    }
};

static const struct TrainerMon sParty_Randall[] = {
    {
    .iv = 255,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_QUICK_ATTACK, MOVE_AGILITY, MOVE_WING_ATTACK, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Parker[] = {
    {
    .iv = 255,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TEETER_DANCE, MOVE_DIZZY_PUNCH, MOVE_FOCUS_PUNCH, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_George[] = {
    {
    .iv = 255,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKOTH,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SLACK_OFF, MOVE_COUNTER, MOVE_SHADOW_BALL, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Berke[] = {
    {
    .iv = 255,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VIGOROTH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FOCUS_ENERGY, MOVE_SLASH, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Braxton[] = {
    {
    .iv = 100,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    .moves = {MOVE_FOCUS_ENERGY, MOVE_QUICK_ATTACK, MOVE_WING_ATTACK, MOVE_ENDEAVOR}
    },
    {
    .iv = 100,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TRAPINCH,
    .moves = {MOVE_BITE, MOVE_DIG, MOVE_FEINT_ATTACK, MOVE_SAND_TOMB}
    },
    {
    .iv = 100,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    .moves = {MOVE_ROLLOUT, MOVE_WHIRLPOOL, MOVE_ASTONISH, MOVE_WATER_PULSE}
    },
    {
    .iv = 100,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_THUNDERBOLT, MOVE_SUPERSONIC, MOVE_THUNDER_WAVE, MOVE_SONIC_BOOM}
    },
    {
    .iv = 100,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHIFTRY,
    .moves = {MOVE_GIGA_DRAIN, MOVE_FEINT_ATTACK, MOVE_DOUBLE_TEAM, MOVE_SWAGGER}
    }
};

static const struct TrainerMon sParty_Vincent[] = {
    {
    .iv = 100,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 100,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 100,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Leroy[] = {
    {
    .iv = 100,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAWILE,
    },
    {
    .iv = 100,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Wilton1[] = {
    {
    .iv = 100,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 100,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 100,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Edgar[] = {
    {
    .iv = 100,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CACTURNE,
    },
    {
    .iv = 100,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMon sParty_Albert[] = {
    {
    .iv = 100,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 100,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MUK,
    }
};

static const struct TrainerMon sParty_Samuel[] = {
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAWILE,
    },
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    }
};

static const struct TrainerMon sParty_Vito[] = {
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODRIO,
    },
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRODE,
    },
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHIFTRY,
    }
};

static const struct TrainerMon sParty_Owen[] = {
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KECLEON,
    },
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILORD,
    }
};

static const struct TrainerMon sParty_Wilton2[] = {
    {
    .iv = 110,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 110,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 110,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Wilton3[] = {
    {
    .iv = 120,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 120,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 120,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Wilton4[] = {
    {
    .iv = 130,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 130,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 130,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Wilton5[] = {
    {
    .iv = 140,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 140,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 140,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Warren[] = {
    {
    .iv = 100,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 100,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUDICOLO,
    }
};

static const struct TrainerMon sParty_Mary[] = {
    {
    .iv = 255,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FEINT_ATTACK, MOVE_SHOCK_WAVE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Alexia[] = {
    {
    .iv = 255,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WIGGLYTUFF,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_DEFENSE_CURL, MOVE_DOUBLE_EDGE, MOVE_SHADOW_BALL, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Jody[] = {
    {
    .iv = 255,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZANGOOSE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SWORDS_DANCE, MOVE_SLASH, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Wendy[] = {
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAWILE,
    .moves = {MOVE_BATON_PASS, MOVE_FEINT_ATTACK, MOVE_FAKE_TEARS, MOVE_BITE}
    },
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_MEGA_DRAIN, MOVE_MAGICAL_LEAF, MOVE_GRASS_WHISTLE, MOVE_LEECH_SEED}
    },
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    .moves = {MOVE_FLY, MOVE_WATER_GUN, MOVE_MIST, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Keira[] = {
    {
    .iv = 100,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = 100,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMon sParty_Brooke1[] = {
    {
    .iv = 100,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 100,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 100,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Jennifer[] = {
    {
    .iv = 200,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMon sParty_Hope[] = {
    {
    .iv = 100,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Shannon[] = {
    {
    .iv = 100,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CLAYDOL,
    }
};

static const struct TrainerMon sParty_Michelle[] = {
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TORKOAL,
    },
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUDICOLO,
    }
};

static const struct TrainerMon sParty_Caroline[] = {
    {
    .iv = 100,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKARMORY,
    },
    {
    .iv = 100,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMon sParty_Julie[] = {
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSLASH,
    },
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINETALES,
    },
    {
    .iv = 100,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    }
};

static const struct TrainerMon sParty_Brooke2[] = {
    {
    .iv = 110,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 110,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 110,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Brooke3[] = {
    {
    .iv = 120,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 120,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 120,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Brooke4[] = {
    {
    .iv = 130,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 130,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 130,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Brooke5[] = {
    {
    .iv = 140,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 140,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = 140,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Patricia[] = {
    {
    .iv = 0,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BANETTE,
    },
    {
    .iv = 0,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMon sParty_Kindra[] = {
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSKULL,
    },
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHUPPET,
    }
};

static const struct TrainerMon sParty_Tammy[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSKULL,
    },
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHUPPET,
    }
};

static const struct TrainerMon sParty_Valerie1[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMon sParty_Tasha[] = {
    {
    .iv = 50,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHUPPET,
    }
};

static const struct TrainerMon sParty_Valerie2[] = {
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPOINK,
    }
};

static const struct TrainerMon sParty_Valerie3[] = {
    {
    .iv = 20,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPOINK,
    },
    {
    .iv = 20,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMon sParty_Valerie4[] = {
    {
    .iv = 30,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPOINK,
    },
    {
    .iv = 30,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SABLEYE,
    }
};

static const struct TrainerMon sParty_Valerie5[] = {
    {
    .iv = 40,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSKULL,
    },
    {
    .iv = 40,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 40,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRUMPIG,
    }
};

static const struct TrainerMon sParty_Cindy1[] = {
    {
    .nickname = _("Geodudette"),
    .iv = 100,
    .lvl = 20,
    .evs = {4, 252, 252, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE_ALOLAN,
    .heldItem = ITEM_METRONOME,
    .moves = {MOVE_SPARK, MOVE_SMACK_DOWN, MOVE_HEADBUTT, MOVE_STEALTH_ROCK},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname = _("Furnoise"),
    .iv = 100,
    .lvl = 20,
    .evs = {4, 0, 252, 0, 252, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_CHARCOAL,
    .moves = {MOVE_CLEAR_SMOG, MOVE_FLAME_WHEEL, MOVE_BULLDOZE, MOVE_BODY_SLAM},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_GREAT_BALL,
    }
};

static const struct TrainerMon sParty_Daphne[] = {
    {
    .iv = 100,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_ATTRACT, MOVE_SWEET_KISS, MOVE_FLAIL, MOVE_WATER_PULSE}
    },
    {
    .iv = 100,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_ATTRACT, MOVE_SAFEGUARD, MOVE_TAKE_DOWN, MOVE_WATER_PULSE}
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter2[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Cindy2[] = {
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_TACKLE, MOVE_TAIL_WHIP, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Brianna[] = {
    {
    .iv = 150,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEAKING,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Naomi[] = {
    {
    .iv = 100,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Cindy3[] = {
    {
    .iv = 10,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Cindy4[] = {
    {
    .iv = 20,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Cindy5[] = {
    {
    .iv = 30,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Cindy6[] = {
    {
    .iv = 40,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_FURY_SWIPES, MOVE_MUD_SPORT, MOVE_ODOR_SLEUTH, MOVE_SAND_ATTACK}
    }
};

static const struct TrainerMon sParty_Melissa[] = {
    {
    .iv = 0,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Sheila[] = {
    {
    .iv = 0,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    }
};

static const struct TrainerMon sParty_Shirley[] = {
    {
    .iv = 0,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Jessica1[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_BIND, MOVE_LICK, MOVE_FURY_SWIPES, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_TAIL, MOVE_SCREECH, MOVE_GLARE, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_Connie[] = {
    {
    .iv = 100,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMon sParty_Bridget[] = {
    {
    .iv = 150,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMon sParty_Olivia[] = {
    {
    .iv = 100,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CLAMPERL,
    .moves = {MOVE_IRON_DEFENSE, MOVE_WHIRLPOOL, MOVE_RAIN_DANCE, MOVE_WATER_PULSE}
    },
    {
    .iv = 100,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CORPHISH,
    .moves = {MOVE_TAUNT, MOVE_CRABHAMMER, MOVE_WATER_PULSE, MOVE_NONE}
    },
    {
    .iv = 100,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    .moves = {MOVE_UPROAR, MOVE_FURY_SWIPES, MOVE_FAKE_OUT, MOVE_WATER_PULSE}
    }
};

static const struct TrainerMon sParty_Tiffany[] = {
    {
    .iv = 100,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    },
    {
    .iv = 100,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Jessica2[] = {
    {
    .iv = 10,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_BIND, MOVE_LICK, MOVE_FURY_SWIPES, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 10,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_TAIL, MOVE_SCREECH, MOVE_GLARE, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_Jessica3[] = {
    {
    .iv = 20,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_BIND, MOVE_LICK, MOVE_FURY_SWIPES, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 20,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_TAIL, MOVE_SCREECH, MOVE_GLARE, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_Jessica4[] = {
    {
    .iv = 30,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_BIND, MOVE_LICK, MOVE_FURY_SWIPES, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 30,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_TAIL, MOVE_SCREECH, MOVE_GLARE, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_Jessica5[] = {
    {
    .iv = 40,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_BIND, MOVE_LICK, MOVE_FURY_SWIPES, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 40,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_POISON_TAIL, MOVE_SCREECH, MOVE_GLARE, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_Winston1[] = {
    {
    .nickname = _("Defense"),
    .iv = 100,
    .lvl = 20,
    .evs = {252, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_UMBREON,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_HELPING_HAND, MOVE_SING, MOVE_CONFUSE_RAY, MOVE_SNARL},
    .nature = NATURE_MODEST,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Offense"),
    .iv = 100,
    .lvl = 20,
    .evs = {0, 0, 0, 252, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_ESPEON,
    .heldItem = ITEM_TWISTED_SPOON,
    .moves = {MOVE_CONFUSION, MOVE_DAZZLING_GLEAM, MOVE_WEATHER_BALL, MOVE_DOUBLE_TEAM},
    .nature = NATURE_MODEST,
    .ball = ITEM_CHERISH_BALL,
    }
};

static const struct TrainerMon sParty_Mollie[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WHISCASH,
    },
    {
    .iv = 200,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMon sParty_Garret[] = {
    {
    .iv = 0,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Winston2[] = {
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Winston3[] = {
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Winston4[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Winston5[] = {
    {
    .iv = 0,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NUGGET,
    .moves = {MOVE_FURY_SWIPES, MOVE_MUD_SPORT, MOVE_ODOR_SLEUTH, MOVE_SAND_ATTACK}
    }
};

static const struct TrainerMon sParty_Steve1[] = {
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARON,
    }
};

static const struct TrainerMon sParty_Thalia1[] = {
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HORSEA,
    }
};

static const struct TrainerMon sParty_Mark[] = {
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RHYHORN,
    }
};

static const struct TrainerMon sParty_GruntMtChimney1[] = {
    {
    .iv = 0,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Steve2[] = {
    {
    .iv = 10,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LAIRON,
    }
};

static const struct TrainerMon sParty_Steve3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RHYHORN,
    }
};

static const struct TrainerMon sParty_Steve4[] = {
    {
    .iv = 30,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = 30,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RHYHORN,
    }
};

static const struct TrainerMon sParty_Steve5[] = {
    {
    .iv = 40,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AGGRON,
    },
    {
    .iv = 40,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RHYDON,
    }
};

static const struct TrainerMon sParty_Luis[] = {
    {
    .nickname = _("Bullseye"),
    .iv = 200,
    .lvl = 25,
    .evs = {0, 100, 0, 0, 0, 75},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_BARRASKEWDA,
    .moves = {MOVE_THROAT_CHOP, MOVE_DIVE, MOVE_ICE_FANG, MOVE_BOUNCE},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Bubbus"),
    .iv = 200,
    .lvl = 25,
    .evs = {0, 100, 0, 0, 0, 75},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GULPIN,
    .moves = {MOVE_TOXIC, MOVE_INFESTATION, MOVE_SLUDGE_WAVE, MOVE_HIDDEN_POWER},
    .nature = NATURE_MODEST,
    .ball = ITEM_ULTRA_BALL,
    }
};

static const struct TrainerMon sParty_Dominik[] = {
    {
    .nickname = _("Stinky"),
    .iv = 200,
    .lvl = 25,
    .evs = {0, 150, 0, 0, 0, 75},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEVIPER,
    .moves = {MOVE_GLARE, MOVE_POISON_FANG, MOVE_SWITCHEROO, MOVE_NIGHT_SLASH},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Stanky"),
    .iv = 200,
    .lvl = 25,
    .evs = {0, 150, 0, 0, 0, 75},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZANGOOSE,
    .moves = {MOVE_SLASH, MOVE_REVENGE, MOVE_FRUSTRATION, MOVE_ICE_BEAM},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_ULTRA_BALL,
    }
};

static const struct TrainerMon sParty_Douglas[] = {
    {
    .nickname = _("Cypress"),
    .iv = 200,
    .lvl = 23,
    .evs = {0, 100, 4, 0, 0, 100},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LEAFEON,
    .moves = {MOVE_MAGICAL_LEAF, MOVE_X_SCISSOR, MOVE_SHADOW_BALL, MOVE_SYNTHESIS},
    .nature = NATURE_MODEST,
    .ball = ITEM_HEAL_BALL,
    },
    {
    .nickname = _("Cirque"),
    .iv = 200,
    .lvl = 23,
    .evs = {0, 0, 0, 100, 4, 100},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLACEON,
    .moves = {MOVE_ICE_FANG, MOVE_HAIL, MOVE_DOUBLE_TEAM, MOVE_EARTHQUAKE},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_HEAL_BALL,
    }
};

static const struct TrainerMon sParty_Darrin[] = {
    {
    .nickname = _("Shellbo"),
    .iv = 200,
    .lvl = 26,
    .evs = {0, 252, 4, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DREDNAW,
    .moves = {MOVE_ROCK_TOMB, MOVE_REVENGE, MOVE_ROCK_BLAST, MOVE_ICE_FANG},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DIVE_BALL,
    },
    {
    .nickname = _("Explo"),
    .iv = 200,
    .lvl = 27,
    .evs = {4, 252, 0, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER_ALOLAN,
    .moves = {MOVE_EXPLOSION, MOVE_EXPLOSION, MOVE_EXPLOSION, MOVE_EXPLOSION},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_HEAVY_BALL,
    },
    {
    .nickname = _("Heracro"),
    .iv = 200,
    .lvl = 28,
    .evs = {252, 0, 252, 0, 4, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HERACROSS,
    .moves = {MOVE_COUNTER, MOVE_AERIAL_ACE, MOVE_VENOSHOCK, MOVE_ROCK_BLAST},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_NET_BALL,
    },
    {
    .nickname = _("Dumbbelldo"),
    .iv = 200,
    .lvl = 29,
    .evs = {252, 252, 4, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    .moves = {MOVE_REVENGE, MOVE_BODY_SLAM, MOVE_PROTECT, MOVE_FLAMETHROWER},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_FRIEND_BALL,
    }
};

static const struct TrainerMon sParty_Tony1[] = {
    {
    .nickname = _("Kombat"),
    .iv = 200,
    .lvl = 26,
    .evs = {4, 100, 0, 0, 0, 100},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_HITMONCHAN,
    .moves = {MOVE_DRAIN_PUNCH, MOVE_FIRE_PUNCH, MOVE_ICE_PUNCH, MOVE_THUNDER_PUNCH},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_PREMIER_BALL,
    },
    {
    .nickname = _("Konquest"),
    .iv = 200,
    .lvl = 26,
    .evs = {4, 100, 0, 0, 0, 100},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_HITMONLEE,
    .moves = {MOVE_DOUBLE_KICK, MOVE_BLAZE_KICK, MOVE_MEGA_KICK, MOVE_STOMPING_TANTRUM},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname = _("Kontend"),
    .iv = 200,
    .lvl = 26,
    .evs = {4, 100, 0, 0, 0, 100},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HITMONTOP,
    .moves = {MOVE_TRIPLE_KICK, MOVE_RAPID_SPIN, MOVE_ROCK_SLIDE, MOVE_BRUTAL_SWING},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_POKE_BALL,
    }
};

static const struct TrainerMon sParty_Jerome[] = {
    {
    .nickname = _("Doug"),
    .iv = 200,
    .lvl = 27,
    .evs = {0, 252, 0, 0, 4, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_DUGTRIO_ALOLAN,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_EARTHQUAKE, MOVE_IRON_HEAD, MOVE_SUBSTITUTE, MOVE_TOXIC},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_TIMER_BALL,
    },
    {
    .nickname = _("Golby"),
    .iv = 200,
    .lvl = 27,
    .evs = {252, 0, 4, 0, 252, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_GOLBAT,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_BRAVE_BIRD, MOVE_TOXIC, MOVE_DEFOG, MOVE_ROOST},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_TIMER_BALL,
    }
};

static const struct TrainerMon sParty_Matthew[] = {
    {
    .nickname = _("Bella"),
    .iv = 200,
    .lvl = 26,
    .evs = {252, 0, 0, 252, 4, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BELLOSSOM,
    .moves = {MOVE_PETAL_BLIZZARD, MOVE_MOONBLAST, MOVE_MOONLIGHT, MOVE_SLEEP_POWDER},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_SPORT_BALL,
    },
    {
    .nickname = _("Villie"),
    .iv = 200,
    .lvl = 26,
    .evs = {252, 0, 252, 0, 4, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_VILEPLUME,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_STRENGTH_SAP, MOVE_SLEEP_POWDER},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_BOLD,
    .ball = ITEM_SPORT_BALL,
    }
};

static const struct TrainerMon sParty_David[] = {
    {
    .nickname = _("Muffin"),
    .iv = 255,
    .lvl = 34,
    .evs = {252, 0, 0, 252, 0, 4},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_VILEPLUME,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB, MOVE_STRENGTH_SAP, MOVE_SLEEP_POWDER},
    .nature = NATURE_BOLD,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Lancelot"),
    .iv = 255,
    .lvl = 35,
    .evs = {252, 0, 168, 0, 88, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_CORVISQUIRE,
    .moves = {MOVE_BODY_PRESS, MOVE_U_TURN, MOVE_DEFOG, MOVE_ROOST},
    .nature = NATURE_IMPISH,
    .ball = ITEM_DUSK_BALL,
    }
};

static const struct TrainerMon sParty_Spencer[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMon sParty_Roland[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_Nolen[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Stan[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HORSEA,
    }
};

static const struct TrainerMon sParty_Barry[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_Dean[] = {
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    },
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_Rodney[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_Richard[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMon sParty_Herman[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Santiago[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Gilbert[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Franklin[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEALEO,
    }
};

static const struct TrainerMon sParty_Kevin[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPHEAL,
    }
};

static const struct TrainerMon sParty_Jack[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_Dudley[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Chad[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Tony2[] = {
    {
    .iv = 10,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Tony3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Tony4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    },
    {
    .iv = 30,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Tony5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 40,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Takao[] = {
    {
    .nickname = _("Jerry"),
    .iv = 200,
    .lvl = 28,
    .evs = {0, 100, 100, 0, 0, 100},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CINCCINO,
    .moves = {MOVE_TAIL_SLAP, MOVE_ROCK_BLAST, MOVE_BULLET_SEED, MOVE_KNOCK_OFF},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_FRIEND_BALL,
    },
    {
    .nickname = _("Bugs"),
    .iv = 200,
    .lvl = 28,
    .evs = {0, 100, 100, 0, 0, 100},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOPUNNY,
    .moves = {MOVE_RETURN, MOVE_HIGH_JUMP_KICK, MOVE_FAKE_OUT, MOVE_QUICK_ATTACK},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_FRIEND_BALL,
    }
};

static const struct TrainerMon sParty_Hitoshi[] = {
    {
    .iv = 50,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 200,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Kiyo[] = {
    {
    .iv = 100,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Koichi[] = {
    {
    .iv = 100,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 150,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Nob1[] = {
    {
    .iv = 100,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMon sParty_Nob2[] = {
    {
    .iv = 110,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Nob3[] = {
    {
    .iv = 120,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 120,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Nob4[] = {
    {
    .iv = 130,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 130,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 130,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Nob5[] = {
    {
    .iv = 140,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 140,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 140,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 140,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_BLACK_BELT
    }
};

static const struct TrainerMon sParty_Yuji[] = {
    {
    .iv = 100,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    },
    {
    .iv = 100,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Daisuke[] = {
    {
    .iv = 100,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMon sParty_Atsushi[] = {
    {
    .iv = 100,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Kirk[] = {
    {
    .nickname = _("Shieldoff"),
    .iv = 100,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHIELDON,
    .moves = {MOVE_TAKE_DOWN, MOVE_SWAGGER, MOVE_INCINERATE, MOVE_SMACK_DOWN},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_HEAVY_BALL,
    },
    {
    .nickname = _("Kevin"),
    .iv = 100,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_DURANT,
    .moves = {MOVE_BITE, MOVE_INFESTATION, MOVE_THUNDER_FANG, MOVE_DIG},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_HEAVY_BALL,
    }
};

static const struct TrainerMon sParty_GruntAquaHideout7[] = {
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_GruntAquaHideout8[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_Shawn[] = {
    {
    .nickname = _("Caliburn"),
    .iv = 100,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HONEDGE,
    .moves = {MOVE_AERIAL_ACE, MOVE_DESTINY_BOND, MOVE_SHOCK_WAVE, MOVE_IRON_HEAD},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_HEAVY_BALL,
    },
    {
    .nickname = _("Trebleklef"),
    .iv = 100,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_KLEFKI,
    .moves = {MOVE_DRAINING_KISS, MOVE_PSYSHOCK, MOVE_CALM_MIND, MOVE_THUNDER_WAVE},
    .nature = NATURE_MODEST,
    .ball = ITEM_HEAVY_BALL,
    }
};

static const struct TrainerMon sParty_Fernando1[] = {
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_Dalton1[] = {
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WHISMUR,
    }
};

static const struct TrainerMon sParty_Dalton2[] = {
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WHISMUR,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Dalton3[] = {
    {
    .iv = 20,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 20,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 20,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Dalton4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMon sParty_Dalton5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    },
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMon sParty_Cole[] = {
    {
    .iv = 100,
    .lvl = 23,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Jeff[] = {
    {
    .iv = 100,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 100,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    }
};

static const struct TrainerMon sParty_Axle[] = {
    {
    .iv = 100,
    .lvl = 23,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Jace[] = {
    {
    .iv = 100,
    .lvl = 23,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    }
};

static const struct TrainerMon sParty_Keegan[] = {
    {
    .iv = 120,
    .lvl = 23,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    }
};

static const struct TrainerMon sParty_Bernie1[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMon sParty_Bernie2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 10,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMon sParty_Bernie3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMon sParty_Bernie4[] = {
    {
    .iv = 30,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 30,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMon sParty_Bernie5[] = {
    {
    .iv = 40,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGCARGO,
    },
    {
    .iv = 40,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMon sParty_Drew[] = {
    {
    .iv = 0,
    .lvl = 23,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    .moves = {MOVE_DIG, MOVE_SAND_ATTACK, MOVE_POISON_STING, MOVE_SLASH}
    }
};

static const struct TrainerMon sParty_Beau[] = {
    {
    .iv = 0,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BALTOY,
    .moves = {MOVE_RAPID_SPIN, MOVE_MUD_SLAP, MOVE_PSYBEAM, MOVE_ROCK_TOMB}
    },
    {
    .iv = 0,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    .moves = {MOVE_POISON_STING, MOVE_SAND_ATTACK, MOVE_SCRATCH, MOVE_DIG}
    },
    {
    .iv = 0,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BALTOY,
    .moves = {MOVE_RAPID_SPIN, MOVE_MUD_SLAP, MOVE_PSYBEAM, MOVE_ROCK_TOMB}
    }
};

static const struct TrainerMon sParty_Larry[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Shane[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Justin[] = {
    {
    .iv = 0,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KECLEON,
    }
};

static const struct TrainerMon sParty_Ethan1[] = {
    {
    .iv = 0,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 0,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TAILLOW,
    }
};

static const struct TrainerMon sParty_Autumn[] = {
    {
    .iv = 0,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    }
};

static const struct TrainerMon sParty_Travis[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    }
};

static const struct TrainerMon sParty_Ethan2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 10,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TAILLOW,
    }
};

static const struct TrainerMon sParty_Ethan3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMon sParty_Ethan4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    }
};

static const struct TrainerMon sParty_Ethan5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSLASH,
    },
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    }
};

static const struct TrainerMon sParty_Brent[] = {
    {
    .iv = 100,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    }
};

static const struct TrainerMon sParty_Donald[] = {
    {
    .iv = 100,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WURMPLE,
    },
    {
    .iv = 100,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SILCOON,
    },
    {
    .iv = 100,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BEAUTIFLY,
    }
};

static const struct TrainerMon sParty_Taylor[] = {
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WURMPLE,
    },
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CASCOON,
    },
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSTOX,
    }
};

static const struct TrainerMon sParty_Jeffrey1[] = {
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    }
};

static const struct TrainerMon sParty_Derek[] = {
    {
    .iv = 150,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = 150,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BEAUTIFLY,
    }
};

static const struct TrainerMon sParty_Jeffrey2[] = {
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    }
};

static const struct TrainerMon sParty_Jeffrey3[] = {
    {
    .iv = 20,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 20,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 20,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MASQUERAIN,
    }
};

static const struct TrainerMon sParty_Jeffrey4[] = {
    {
    .iv = 30,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 30,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WURMPLE,
    },
    {
    .iv = 30,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 30,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MASQUERAIN,
    }
};

static const struct TrainerMon sParty_Jeffrey5[] = {
    {
    .iv = 40,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 40,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSTOX,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 40,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 40,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MASQUERAIN,
    .heldItem = ITEM_SILVER_POWDER
    },
    {
    .iv = 40,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BEAUTIFLY,
    .heldItem = ITEM_NONE
    }
};

static const struct TrainerMon sParty_Edward[] = {
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ABRA,
    .moves = {MOVE_HIDDEN_POWER, MOVE_NONE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Preston[] = {
    {
    .iv = 100,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KIRLIA,
    }
};

static const struct TrainerMon sParty_Virgil[] = {
    {
    .iv = 100,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RALTS,
    }
};

static const struct TrainerMon sParty_Blake[] = {
    {
    .iv = 100,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GIRAFARIG,
    }
};

static const struct TrainerMon sParty_William[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RALTS,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RALTS,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KIRLIA,
    }
};

static const struct TrainerMon sParty_Joshua[] = {
    {
    .iv = 0,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 0,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMon sParty_Cameron1[] = {
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMon sParty_Cameron2[] = {
    {
    .iv = 10,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 10,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMon sParty_Cameron3[] = {
    {
    .iv = 20,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 20,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMon sParty_Cameron4[] = {
    {
    .iv = 30,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 30,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SOLROCK,
    }
};

static const struct TrainerMon sParty_Cameron5[] = {
    {
    .iv = 40,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SOLROCK,
    },
    {
    .iv = 40,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALAKAZAM,
    }
};

static const struct TrainerMon sParty_Jaclyn[] = {
    {
    .iv = 0,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ABRA,
    .moves = {MOVE_HIDDEN_POWER, MOVE_NONE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Hannah[] = {
    {
    .iv = 100,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KIRLIA,
    }
};

static const struct TrainerMon sParty_Samantha[] = {
    {
    .iv = 100,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMon sParty_Maura[] = {
    {
    .iv = 100,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    }
};

static const struct TrainerMon sParty_Kayla[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WOBBUFFET,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NATU,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    }
};

static const struct TrainerMon sParty_Alexis[] = {
    {
    .iv = 0,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KIRLIA,
    },
    {
    .iv = 0,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMon sParty_Jacki1[] = {
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMon sParty_Jacki2[] = {
    {
    .iv = 10,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 10,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMon sParty_Jacki3[] = {
    {
    .iv = 20,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 20,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMon sParty_Jacki4[] = {
    {
    .iv = 30,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 30,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUNATONE,
    }
};

static const struct TrainerMon sParty_Jacki5[] = {
    {
    .iv = 40,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUNATONE,
    },
    {
    .iv = 40,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALAKAZAM,
    }
};

static const struct TrainerMon sParty_Walter1[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMon sParty_Micah[] = {
    {
    .iv = 0,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 0,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMon sParty_Thomas[] = {
    {
    .iv = 0,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZANGOOSE,
    }
};

static const struct TrainerMon sParty_Walter2[] = {
    {
    .iv = 10,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMon sParty_Walter3[] = {
    {
    .iv = 20,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_HEADBUTT, MOVE_SAND_ATTACK, MOVE_ODOR_SLEUTH, MOVE_FURY_SWIPES}
    },
    {
    .iv = 20,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_QUICK_ATTACK, MOVE_SPARK, MOVE_ODOR_SLEUTH, MOVE_ROAR}
    }
};

static const struct TrainerMon sParty_Walter4[] = {
    {
    .iv = 30,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_HEADBUTT, MOVE_SAND_ATTACK, MOVE_ODOR_SLEUTH, MOVE_FURY_SWIPES}
    },
    {
    .iv = 30,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_QUICK_ATTACK, MOVE_SPARK, MOVE_ODOR_SLEUTH, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Walter5[] = {
    {
    .iv = 40,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_HEADBUTT, MOVE_SAND_ATTACK, MOVE_ODOR_SLEUTH, MOVE_FURY_SWIPES}
    },
    {
    .iv = 40,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLDUCK,
    .moves = {MOVE_FURY_SWIPES, MOVE_DISABLE, MOVE_CONFUSION, MOVE_PSYCH_UP}
    },
    {
    .iv = 40,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_QUICK_ATTACK, MOVE_SPARK, MOVE_ODOR_SLEUTH, MOVE_ROAR}
    }
};

static const struct TrainerMon sParty_Sidney[] = {
    {
    .iv = 250,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ROAR, MOVE_DOUBLE_EDGE, MOVE_SAND_ATTACK, MOVE_CRUNCH}
    },
    {
    .iv = 250,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHIFTRY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TORMENT, MOVE_DOUBLE_TEAM, MOVE_SWAGGER, MOVE_EXTRASENSORY}
    },
    {
    .iv = 250,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CACTURNE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_LEECH_SEED, MOVE_FEINT_ATTACK, MOVE_NEEDLE_ARM, MOVE_COTTON_SPORE}
    },
    {
    .iv = 250,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_SWORDS_DANCE, MOVE_STRENGTH, MOVE_FACADE}
    },
    {
    .iv = 255,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ABSOL,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_AERIAL_ACE, MOVE_ROCK_SLIDE, MOVE_SWORDS_DANCE, MOVE_SLASH}
    }
};

static const struct TrainerMon sParty_Phoebe[] = {
    {
    .iv = 250,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSCLOPS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SHADOW_PUNCH, MOVE_CONFUSE_RAY, MOVE_CURSE, MOVE_PROTECT}
    },
    {
    .iv = 250,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BANETTE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SHADOW_BALL, MOVE_GRUDGE, MOVE_WILL_O_WISP, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 250,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SABLEYE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SHADOW_BALL, MOVE_DOUBLE_TEAM, MOVE_NIGHT_SHADE, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 250,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BANETTE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SHADOW_BALL, MOVE_PSYCHIC, MOVE_THUNDERBOLT, MOVE_FACADE}
    },
    {
    .iv = 255,
    .lvl = 51,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSCLOPS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SHADOW_BALL, MOVE_ICE_BEAM, MOVE_ROCK_SLIDE, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMon sParty_Glacia[] = {
    {
    .iv = 250,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEALEO,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ENCORE, MOVE_BODY_SLAM, MOVE_HAIL, MOVE_ICE_BALL}
    },
    {
    .iv = 250,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLALIE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_CRUNCH, MOVE_ICY_WIND, MOVE_ICE_BEAM}
    },
    {
    .iv = 250,
    .lvl = 52,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEALEO,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ATTRACT, MOVE_DOUBLE_EDGE, MOVE_HAIL, MOVE_BLIZZARD}
    },
    {
    .iv = 250,
    .lvl = 52,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLALIE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SHADOW_BALL, MOVE_EXPLOSION, MOVE_HAIL, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SURF, MOVE_BODY_SLAM, MOVE_ICE_BEAM, MOVE_SHEER_COLD}
    }
};

static const struct TrainerMon sParty_Drake[] = {
    {
    .iv = 250,
    .lvl = 52,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHELGON,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ROCK_TOMB, MOVE_DRAGON_CLAW, MOVE_PROTECT, MOVE_DOUBLE_EDGE}
    },
    {
    .iv = 250,
    .lvl = 54,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_DOUBLE_EDGE, MOVE_DRAGON_BREATH, MOVE_DRAGON_DANCE, MOVE_AERIAL_ACE}
    },
    {
    .iv = 250,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SMOKESCREEN, MOVE_DRAGON_DANCE, MOVE_SURF, MOVE_BODY_SLAM}
    },
    {
    .iv = 250,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_FLYGON,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FLAMETHROWER, MOVE_CRUNCH, MOVE_DRAGON_BREATH, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 55,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SALAMENCE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_FLAMETHROWER, MOVE_DRAGON_CLAW, MOVE_ROCK_SLIDE, MOVE_CRUNCH}
    }
};

static const struct TrainerMon sParty_Roxanne1[] = {
    {
    .nickname = _("Wyborn"),
    .iv = 255,
    .lvl = 15,
    .evs = {0, 252, 4, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIMIKYU,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_HEX, MOVE_THUNDER_WAVE, MOVE_SHADOW_SNEAK, MOVE_DOUBLE_TEAM},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_ULTRA_BALL,
    .shiny = TRUE,
    },
    {
    .nickname = _("Coraline"),
    .iv = 255,
    .lvl = 16,
    .evs = {0, 0, 0, 252, 4, 252},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_CORSOLA_GALARIAN,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_HEX, MOVE_GIGA_DRAIN, MOVE_RAPID_SPIN, MOVE_RECOVER},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("The Beldam"),
    .iv = 255,
    .lvl = 18,
    .evs = {248, 0, 8, 0, 252, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SABLEYE,
    .heldItem = ITEM_SABLENITE,
    .moves = {MOVE_HEX, MOVE_ICY_WIND, MOVE_DIG, MOVE_FIRE_PUNCH},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_RELAXED,
    .ball = ITEM_MASTER_BALL,
    }
};

static const struct TrainerMon sParty_Brawly1[] = {
    {
    .nickname = _("Wigglekins"),
    .iv = 255,
    .lvl = 29,
    .evs = {4, 0, 0, 252, 0, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_WIGGLYTUFF,
    .heldItem = ITEM_ROCKY_HELMET,
    .moves = {MOVE_ATTRACT, MOVE_DRAINING_KISS, MOVE_STEALTH_ROCK, MOVE_FIRE_BLAST},
    .nature = NATURE_TIMID,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("Scwatchy"),
    .iv = 255,
    .lvl = 30,
    .evs = {0, 252, 4, 0, 0, 252},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_ZANGOOSE,
    .heldItem = ITEM_TOXIC_ORB,
    .moves = {MOVE_ATTRACT, MOVE_CLOSE_COMBAT, MOVE_SWORDS_DANCE, MOVE_FACADE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("Fuzzywuzzy"),
    .iv = 255,
    .lvl = 30,
    .evs = {188, 0, 128, 0, 192, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_SNORLAX,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_ATTRACT, MOVE_BODY_SLAM, MOVE_CURSE, MOVE_EARTHQUAKE},
    .nature = NATURE_CAREFUL,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("Scawybiwrd"),
    .iv = 255,
    .lvl = 31,
    .evs = {4, 252, 0, 0, 0, 252},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_SWELLOW,
    .heldItem = ITEM_FOCUS_SASH,
    .moves = {MOVE_ATTRACT, MOVE_BOOMBURST, MOVE_HAZE, MOVE_AIR_SLASH},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("Fwuffyball"),
    .iv = 255,
    .lvl = 32,
    .evs = {104, 0, 252, 0, 0, 152},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUBWOOL,
    .heldItem = ITEM_QUICK_CLAW,
    .moves = {MOVE_ZEN_HEADBUTT, MOVE_BODY_PRESS, MOVE_ATTRACT, MOVE_PAYBACK},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("Huggymommy"),
    .iv = 255,
    .lvl = 33,
    .evs = {52, 180, 36, 0, 8, 232},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_KANGASKHANITE,
    .moves = {MOVE_ATTRACT, MOVE_LOW_KICK, MOVE_CRUNCH, MOVE_DOUBLE_EDGE},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_MASTER_BALL,
    }
};

static const struct TrainerMon sParty_Wattson1[] = {
    {
    .nickname = _("MetPet"),
    .iv = 255,
    .lvl = 23,
    .evs = {252, 0, 121, 0, 131, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_METANG,
    .heldItem = ITEM_ASSAULT_VEST,
    .moves = {MOVE_IRON_DEFENSE, MOVE_MUD_SHOT, MOVE_ICE_PUNCH, MOVE_ZEN_HEADBUTT},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("AronHanson"),
    .iv = 255,
    .lvl = 24,
    .evs = {4, 0, 252, 0, 252, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARON,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_IRON_DEFENSE, MOVE_IRON_TAIL, MOVE_STEALTH_ROCK, MOVE_BULLDOZE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_IMPISH,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("Pewdiepass"),
    .iv = 255,
    .lvl = 24,
    .evs = {172, 0, 0, 252, 0, 84},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_PROBOPASS,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_SMACK_DOWN, MOVE_VOLT_SWITCH, MOVE_IRON_DEFENSE, MOVE_DAZZLING_GLEAM},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_LAX,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("Mawliplier"),
    .iv = 255,
    .lvl = 25,
    .evs = {92, 252, 0, 0, 0, 164},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_MAWILE,
    .heldItem = ITEM_MAWILITE,
    .moves = {MOVE_IRON_DEFENSE, MOVE_ICE_PUNCH, MOVE_DRAINING_KISS, MOVE_ROCK_SLIDE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_LONELY,
    .ball = ITEM_MASTER_BALL,
    }
};

static const struct TrainerMon sParty_Flannery1[] = {
    {
    .iv = 200,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_OVERHEAT, MOVE_TAKE_DOWN, MOVE_MAGNITUDE, MOVE_SUNNY_DAY}
    },
    {
    .iv = 200,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_OVERHEAT, MOVE_SMOG, MOVE_LIGHT_SCREEN, MOVE_SUNNY_DAY}
    },
    {
    .iv = 250,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_OVERHEAT, MOVE_TACKLE, MOVE_SUNNY_DAY, MOVE_ATTRACT}
    },
    {
    .iv = 250,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_BODY_SLAM, MOVE_ATTRACT}
    }
};

static const struct TrainerMon sParty_Norman1[] = {
    {
    .iv = 200,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TEETER_DANCE, MOVE_PSYBEAM, MOVE_FACADE, MOVE_ENCORE}
    },
    {
    .iv = 200,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VIGOROTH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SLASH, MOVE_FACADE, MOVE_ENCORE, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 200,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SLASH, MOVE_BELLY_DRUM, MOVE_FACADE, MOVE_HEADBUTT}
    },
    {
    .iv = 250,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_COUNTER, MOVE_YAWN, MOVE_FACADE, MOVE_FEINT_ATTACK}
    }
};

static const struct TrainerMon sParty_Winona1[] = {
    {
    .iv = 210,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PERISH_SONG, MOVE_MIRROR_MOVE, MOVE_SAFEGUARD, MOVE_AERIAL_ACE}
    },
    {
    .iv = 210,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SUNNY_DAY, MOVE_AERIAL_ACE, MOVE_SOLAR_BEAM, MOVE_SYNTHESIS}
    },
    {
    .iv = 210,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATER_GUN, MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_AERIAL_ACE}
    },
    {
    .iv = 220,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SAND_ATTACK, MOVE_FURY_ATTACK, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_DRAGON_BREATH, MOVE_DRAGON_DANCE, MOVE_AERIAL_ACE}
    }
};

static const struct TrainerMon sParty_TateAndLiza1[] = {
    {
    .iv = 250,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_EARTHQUAKE, MOVE_ANCIENT_POWER, MOVE_PSYCHIC, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 250,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_XATU,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PSYCHIC, MOVE_SUNNY_DAY, MOVE_CONFUSE_RAY, MOVE_CALM_MIND}
    },
    {
    .iv = 250,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_PSYCHIC, MOVE_HYPNOSIS, MOVE_CALM_MIND}
    },
    {
    .iv = 250,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM, MOVE_PSYCHIC, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMon sParty_Juan1[] = {
    {
    .iv = 200,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATER_PULSE, MOVE_ATTRACT, MOVE_SWEET_KISS, MOVE_FLAIL}
    },
    {
    .iv = 200,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_PULSE, MOVE_AMNESIA, MOVE_EARTHQUAKE}
    },
    {
    .iv = 200,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEALEO,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ENCORE, MOVE_BODY_SLAM, MOVE_AURORA_BEAM, MOVE_WATER_PULSE}
    },
    {
    .iv = 200,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATER_PULSE, MOVE_CRABHAMMER, MOVE_TAUNT, MOVE_LEER}
    },
    {
    .iv = 250,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMon sParty_Jerry1[] = {
    {
    .iv = 10,
    .lvl = 9,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RALTS,
    }
};

static const struct TrainerMon sParty_Ted[] = {
    {
    .iv = 10,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RALTS,
    }
};

static const struct TrainerMon sParty_Paul2[] = {
    {
    .iv = 10,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 10,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ODDISH,
    },
    {
    .iv = 10,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMon sParty_Jerry2[] = {
    {
    .iv = 20,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RALTS,
    },
    {
    .iv = 20,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMon sParty_Jerry3[] = {
    {
    .iv = 30,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KIRLIA,
    },
    {
    .iv = 30,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMon sParty_Jerry4[] = {
    {
    .iv = 40,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KIRLIA,
    },
    {
    .iv = 40,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMon sParty_Jerry5[] = {
    {
    .iv = 50,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KIRLIA,
    },
    {
    .iv = 50,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BANETTE,
    },
    {
    .iv = 50,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMon sParty_Karen1[] = {
    {
    .iv = 10,
    .lvl = 9,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    }
};

static const struct TrainerMon sParty_Georgia[] = {
    {
    .iv = 10,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 10,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BEAUTIFLY,
    }
};

static const struct TrainerMon sParty_Karen2[] = {
    {
    .iv = 20,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 20,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WHISMUR,
    }
};

static const struct TrainerMon sParty_Karen3[] = {
    {
    .iv = 30,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 30,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_Karen4[] = {
    {
    .iv = 40,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 40,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_Karen5[] = {
    {
    .iv = 50,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 50,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_EXPLOUD,
    }
};

static const struct TrainerMon sParty_KateAndJoy[] = {
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPINDA,
    .moves = {MOVE_HYPNOSIS, MOVE_PSYBEAM, MOVE_DIZZY_PUNCH, MOVE_TEETER_DANCE}
    },
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    .moves = {MOVE_FOCUS_PUNCH, MOVE_YAWN, MOVE_SLACK_OFF, MOVE_FEINT_ATTACK}
    }
};

static const struct TrainerMon sParty_AnnaAndMeg1[] = {
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    .moves = {MOVE_GROWL, MOVE_TAIL_WHIP, MOVE_HEADBUTT, MOVE_ODOR_SLEUTH}
    },
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    .moves = {MOVE_TACKLE, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_AnnaAndMeg2[] = {
    {
    .iv = 10,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    .moves = {MOVE_GROWL, MOVE_TAIL_WHIP, MOVE_HEADBUTT, MOVE_ODOR_SLEUTH}
    },
    {
    .iv = 10,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    .moves = {MOVE_TACKLE, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_AnnaAndMeg3[] = {
    {
    .iv = 20,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    .moves = {MOVE_GROWL, MOVE_TAIL_WHIP, MOVE_HEADBUTT, MOVE_ODOR_SLEUTH}
    },
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    .moves = {MOVE_TACKLE, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_AnnaAndMeg4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_GROWL, MOVE_TAIL_WHIP, MOVE_HEADBUTT, MOVE_ODOR_SLEUTH}
    },
    {
    .iv = 30,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    .moves = {MOVE_TACKLE, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_AnnaAndMeg5[] = {
    {
    .iv = 40,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .moves = {MOVE_GROWL, MOVE_TAIL_WHIP, MOVE_HEADBUTT, MOVE_ODOR_SLEUTH}
    },
    {
    .iv = 40,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_TACKLE, MOVE_FOCUS_ENERGY, MOVE_ARM_THRUST, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Victor[] = {
    {
    .iv = 25,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TAILLOW,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 25,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Miguel1[] = {
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Colton[] = {
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK, MOVE_HEAL_BELL}
    },
    {
    .iv = 0,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK, MOVE_HEAL_BELL}
    },
    {
    .iv = 0,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK, MOVE_HEAL_BELL}
    },
    {
    .iv = 0,
    .lvl = 12,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK, MOVE_HEAL_BELL}
    },
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK, MOVE_HEAL_BELL}
    },
    {
    .iv = 0,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK, MOVE_HEAL_BELL}
    }
};

static const struct TrainerMon sParty_Miguel2[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Miguel3[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Miguel4[] = {
    {
    .iv = 0,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Miguel5[] = {
    {
    .iv = 0,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMon sParty_Victoria[] = {
    {
    .iv = 50,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Vanessa[] = {
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PIKACHU,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Bethany[] = {
    {
    .iv = 100,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AZURILL,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 100,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 100,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AZUMARILL,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Isabel1[] = {
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Isabel2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 10,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Isabel3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Isabel4[] = {
    {
    .iv = 30,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 30,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Isabel5[] = {
    {
    .iv = 40,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_SITRUS_BERRY
    },
    {
    .iv = 40,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_SITRUS_BERRY
    }
};

static const struct TrainerMon sParty_Timothy1[] = {
    {
    .iv = 200,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Timothy2[] = {
    {
    .iv = 210,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_ARM_THRUST, MOVE_KNOCK_OFF, MOVE_SAND_ATTACK, MOVE_DIG}
    }
};

static const struct TrainerMon sParty_Timothy3[] = {
    {
    .iv = 220,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_ARM_THRUST, MOVE_KNOCK_OFF, MOVE_SAND_ATTACK, MOVE_DIG}
    }
};

static const struct TrainerMon sParty_Timothy4[] = {
    {
    .iv = 230,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_ARM_THRUST, MOVE_BELLY_DRUM, MOVE_SAND_ATTACK, MOVE_DIG}
    }
};

static const struct TrainerMon sParty_Timothy5[] = {
    {
    .iv = 240,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_ARM_THRUST, MOVE_BELLY_DRUM, MOVE_SAND_ATTACK, MOVE_DIG}
    }
};

static const struct TrainerMon sParty_Vicky[] = {
    {
    .iv = 200,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    .moves = {MOVE_HIGH_JUMP_KICK, MOVE_MEDITATE, MOVE_CONFUSION, MOVE_DETECT}
    }
};

static const struct TrainerMon sParty_Shelby1[] = {
    {
    .iv = 200,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 200,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Shelby2[] = {
    {
    .iv = 210,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 210,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Shelby3[] = {
    {
    .iv = 220,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 220,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Shelby4[] = {
    {
    .iv = 230,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 230,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Shelby5[] = {
    {
    .iv = 240,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 240,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Calvin1[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    }
};

static const struct TrainerMon sParty_Billy[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 0,
    .lvl = 7,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEEDOT,
    }
};

static const struct TrainerMon sParty_Josh[] = {
    {
    .nickname = _("Shadcastle"),
    .iv = 100,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDYGAST,
    .moves = {MOVE_ABSORB, MOVE_HARDEN, MOVE_ROCK_TOMB, MOVE_SAND_ATTACK},
    .nature = NATURE_RELAXED,
    .ball = ITEM_DUSK_BALL,
    .shiny = 1,
    },
    {
    .nickname = _("Earl Grave"),
    .iv = 100,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SINISTEA,
    .moves = {MOVE_WONDER_ROOM, MOVE_HEX, MOVE_MEGA_DRAIN, MOVE_WILL_O_WISP},
    .nature = NATURE_MODEST,
    .ball = ITEM_DUSK_BALL,
    }
};

static const struct TrainerMon sParty_Tommy[] = {
    {
    .nickname = _("Mr. Spooks"),
    .iv = 110,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GASTLY,
    .moves = {MOVE_LICK, MOVE_HYPNOSIS, MOVE_ICY_WIND, MOVE_HEX},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_SAFARI_BALL,
    },
    {
    .nickname = _("Jack-O-Boo"),
    .iv = 120,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PUMPKABOO,
    .moves = {MOVE_TRICK_OR_TREAT, MOVE_CONFUSE_RAY, MOVE_LEECH_SEED, MOVE_SHADOW_SNEAK},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_SAFARI_BALL,
    }
};

static const struct TrainerMon sParty_Joey[] = {
    {
    .iv = 0,
    .lvl = 9,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMon sParty_Ben[] = {
    {
    .nickname = _("Gearfield"),
    .iv = 150,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_KLINK,
    .moves = {MOVE_THUNDER_SHOCK, MOVE_THUNDER_WAVE, MOVE_POWER_GEM, MOVE_SANDSTORM},
    .nature = NATURE_SASSY,
    .ball = ITEM_HEAVY_BALL,
    },
    {
    .nickname = _("Galarfield"),
    .iv = 150,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_MEOWTH_GALARIAN,
    .moves = {MOVE_PLAY_ROUGH, MOVE_HONE_CLAWS, MOVE_PAY_DAY, MOVE_PROTECT},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_HEAVY_BALL,
    }
};

static const struct TrainerMon sParty_Quincy[] = {
    {
    .iv = 100,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    .moves = {MOVE_ATTRACT, MOVE_ICE_BEAM, MOVE_THUNDERBOLT, MOVE_FLAMETHROWER}
    },
    {
    .iv = 100,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSCLOPS,
    .moves = {MOVE_SKILL_SWAP, MOVE_PROTECT, MOVE_WILL_O_WISP, MOVE_TOXIC}
    }
};

static const struct TrainerMon sParty_Katelynn[] = {
    {
    .iv = 100,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GARDEVOIR,
    .moves = {MOVE_SKILL_SWAP, MOVE_PSYCHIC, MOVE_THUNDERBOLT, MOVE_CALM_MIND}
    },
    {
    .iv = 100,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    .moves = {MOVE_EARTHQUAKE, MOVE_SHADOW_BALL, MOVE_AERIAL_ACE, MOVE_BRICK_BREAK}
    }
};

static const struct TrainerMon sParty_Jaylen[] = {
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TRAPINCH,
    }
};

static const struct TrainerMon sParty_Dillon[] = {
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARON,
    }
};

static const struct TrainerMon sParty_Calvin2[] = {
    {
    .iv = 10,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMon sParty_Calvin3[] = {
    {
    .iv = 20,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 20,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMon sParty_Calvin4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 30,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 30,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMon sParty_Calvin5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 40,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 40,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMon sParty_Eddie[] = {
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 0,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    }
};

static const struct TrainerMon sParty_Allen[] = {
    {
    .iv = 0,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 0,
    .lvl = 3,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TAILLOW,
    }
};

static const struct TrainerMon sParty_Timmy2[] = {
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARON,
    },
    {
    .iv = 0,
    .lvl = 13,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRIKE,
    }
};

static const struct TrainerMon sParty_Wallace[] = {
    {
    .iv = 255,
    .lvl = 57,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILORD,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_SPOUT, MOVE_DOUBLE_EDGE, MOVE_BLIZZARD}
    },
    {
    .iv = 255,
    .lvl = 55,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TOXIC, MOVE_HYDRO_PUMP, MOVE_SLUDGE_BOMB, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 56,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUDICOLO,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_GIGA_DRAIN, MOVE_SURF, MOVE_LEECH_SEED, MOVE_DOUBLE_TEAM}
    },
    {
    .iv = 255,
    .lvl = 56,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_EARTHQUAKE, MOVE_SURF, MOVE_AMNESIA, MOVE_HYPER_BEAM}
    },
    {
    .iv = 255,
    .lvl = 56,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE, MOVE_HYPER_BEAM, MOVE_SURF}
    },
    {
    .iv = 255,
    .lvl = 58,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MILOTIC,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_RECOVER, MOVE_SURF, MOVE_ICE_BEAM, MOVE_TOXIC}
    }
};

static const struct TrainerMon sParty_Andrew[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGIKARP,
    },
    {
    .iv = 0,
    .lvl = 10,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGIKARP,
    }
};

static const struct TrainerMon sParty_Ivan[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGIKARP,
    },
    {
    .iv = 0,
    .lvl = 6,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGIKARP,
    },
    {
    .iv = 0,
    .lvl = 7,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGIKARP,
    }
};

static const struct TrainerMon sParty_Claude[] = {
    {
    .iv = 0,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGIKARP,
    },
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLDEEN,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BARBOACH,
    }
};

static const struct TrainerMon sParty_Elliot1[] = {
    {
    .iv = 0,
    .lvl = 10,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGIKARP,
    },
    {
    .iv = 0,
    .lvl = 7,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 10,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGIKARP,
    }
};

static const struct TrainerMon sParty_Ned[] = {
    {
    .iv = 10,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMon sParty_Dale[] = {
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Nolan[] = {
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BARBOACH,
    }
};

static const struct TrainerMon sParty_Barny[] = {
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_Wade[] = {
    {
    .iv = 0,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMon sParty_Carter[] = {
    {
    .nickname = _("Vivi"),
    .iv = 255,
    .lvl = 34,
    .evs = {252, 0, 248, 0, 0, 8},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_LUMINEON,
    .moves = {MOVE_DEFOG, MOVE_SCALD, MOVE_U_TURN, MOVE_TOXIC},
    .nature = NATURE_BOLD,
    .ball = ITEM_LURE_BALL,
    },
    {
    .nickname = _("Wiwiwi"),
    .iv = 255,
    .lvl = 34,
    .evs = {252, 0, 4, 252, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WISHIWASHI,
    .moves = {MOVE_HYDRO_PUMP, MOVE_ICE_BEAM, MOVE_HIDDEN_POWER, MOVE_U_TURN},
    .nature = NATURE_QUIET,
    .ball = ITEM_LURE_BALL,
    }
};

static const struct TrainerMon sParty_Elliot2[] = {
    {
    .iv = 10,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 10,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 10,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_Elliot3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 20,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    },
    {
    .iv = 20,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_Elliot4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 30,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    },
    {
    .iv = 30,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 31,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_Elliot5[] = {
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    },
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 40,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Ronald[] = {
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGIKARP,
    },
    {
    .iv = 0,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 0,
    .lvl = 23,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    },
    {
    .iv = 0,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_Jacob[] = {
    {
    .iv = 20,
    .lvl = 6,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VOLTORB,
    },
    {
    .iv = 20,
    .lvl = 6,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VOLTORB,
    },
    {
    .iv = 200,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Anthony[] = {
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Benjamin1[] = {
    {
    .iv = 0,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Benjamin2[] = {
    {
    .iv = 10,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Benjamin3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Benjamin4[] = {
    {
    .iv = 30,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMon sParty_Benjamin5[] = {
    {
    .iv = 40,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMon sParty_Abigail1[] = {
    {
    .iv = 0,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Jasmine[] = {
    {
    .iv = 80,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 80,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    },
    {
    .iv = 0,
    .lvl = 6,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VOLTORB,
    }
};

static const struct TrainerMon sParty_Abigail2[] = {
    {
    .iv = 10,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Abigail3[] = {
    {
    .iv = 20,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Abigail4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMon sParty_Abigail5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    }
};

static const struct TrainerMon sParty_Dylan1[] = {
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMon sParty_Dylan2[] = {
    {
    .iv = 10,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMon sParty_Dylan3[] = {
    {
    .iv = 20,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMon sParty_Dylan4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMon sParty_Dylan5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMon sParty_Maria1[] = {
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMon sParty_Maria2[] = {
    {
    .iv = 10,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMon sParty_Maria3[] = {
    {
    .iv = 20,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMon sParty_Maria4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMon sParty_Maria5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMon sParty_Camden[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Demetrius[] = {
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRIKE,
    }
};

static const struct TrainerMon sParty_Isaiah1[] = {
    {
    .iv = 0,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Pablo1[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Chase[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 80,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Isaiah2[] = {
    {
    .iv = 10,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Isaiah3[] = {
    {
    .iv = 20,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Isaiah4[] = {
    {
    .iv = 30,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Isaiah5[] = {
    {
    .iv = 40,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Isobel[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Donny[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 160,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Talia[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Katelyn1[] = {
    {
    .iv = 0,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Allison[] = {
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 240,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Katelyn2[] = {
    {
    .iv = 10,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Katelyn3[] = {
    {
    .iv = 20,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Katelyn4[] = {
    {
    .iv = 30,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Katelyn5[] = {
    {
    .iv = 40,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Nicolas1[] = {
    {
    .iv = 100,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = 100,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    }
};

static const struct TrainerMon sParty_Nicolas2[] = {
    {
    .iv = 110,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = 110,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    }
};

static const struct TrainerMon sParty_Nicolas3[] = {
    {
    .iv = 120,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = 120,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    }
};

static const struct TrainerMon sParty_Nicolas4[] = {
    {
    .iv = 130,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BAGON,
    },
    {
    .iv = 130,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = 130,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    }
};

static const struct TrainerMon sParty_Nicolas5[] = {
    {
    .iv = 140,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 140,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 140,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHELGON,
    .heldItem = ITEM_DRAGON_FANG
    }
};

static const struct TrainerMon sParty_Aaron[] = {
    {
    .iv = 255,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BAGON,
    .moves = {MOVE_DRAGON_BREATH, MOVE_HEADBUTT, MOVE_FOCUS_ENERGY, MOVE_EMBER}
    }
};

static const struct TrainerMon sParty_Perry[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMon sParty_Hugh[] = {
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    }
};

static const struct TrainerMon sParty_Phil[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMon sParty_Jared[] = {
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODUO,
    },
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKARMORY,
    },
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    }
};

static const struct TrainerMon sParty_Humberto[] = {
    {
    .iv = 250,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKARMORY,
    }
};

static const struct TrainerMon sParty_Presley[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMon sParty_Edwardo[] = {
    {
    .iv = 150,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODUO,
    },
    {
    .iv = 150,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMon sParty_Colin[] = {
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NATU,
    }
};

static const struct TrainerMon sParty_Robert1[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMon sParty_Benny[] = {
    {
    .iv = 0,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 0,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 0,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMon sParty_Chester[] = {
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMon sParty_Robert2[] = {
    {
    .iv = 10,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NATU,
    },
    {
    .iv = 10,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMon sParty_Robert3[] = {
    {
    .iv = 20,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NATU,
    },
    {
    .iv = 20,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    }
};

static const struct TrainerMon sParty_Robert4[] = {
    {
    .iv = 30,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NATU,
    },
    {
    .iv = 30,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    }
};

static const struct TrainerMon sParty_Robert5[] = {
    {
    .iv = 40,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    },
    {
    .iv = 40,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMon sParty_Alex[] = {
    {
    .iv = 150,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NATU,
    },
    {
    .iv = 150,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMon sParty_Beck[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    }
};

static const struct TrainerMon sParty_Yasu[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_Takashi[] = {
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINJASK,
    },
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    }
};

static const struct TrainerMon sParty_Dianne[] = {
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SKILL_SWAP, MOVE_EARTHQUAKE, MOVE_NONE, MOVE_NONE}
    },
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_THUNDERBOLT, MOVE_EARTHQUAKE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Jani[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Lao1[] = {
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SMOG, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SMOG, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    }
};

static const struct TrainerMon sParty_Lung[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_Lao2[] = {
    {
    .iv = 0,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 0,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE}
    },
    {
    .iv = 0,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Lao3[] = {
    {
    .iv = 20,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 20,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 20,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE}
    },
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Lao4[] = {
    {
    .iv = 30,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE}
    },
    {
    .iv = 30,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE}
    },
    {
    .iv = 30,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE}
    },
    {
    .iv = 30,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Lao5[] = {
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE}
    },
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_POISON_GAS, MOVE_TACKLE, MOVE_SLUDGE, MOVE_SELF_DESTRUCT}
    },
    {
    .iv = 40,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WEEZING,
    .heldItem = ITEM_SMOKE_BALL,
    .moves = {MOVE_TACKLE, MOVE_SLUDGE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Jocelyn[] = {
    {
    .nickname = _("Dittabo"),
    .iv = 200,
    .lvl = 28,
    .evs = {248, 8, 252, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_DITTO,
    .ball = ITEM_FRIEND_BALL,
    },
    {
    .nickname = _("Dittabee"),
    .iv = 200,
    .lvl = 28,
    .evs = {248, 8, 252, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_DITTO,
    .ball = ITEM_FRIEND_BALL,
    },
    {
    .nickname = _("Dittaboo"),
    .iv = 200,
    .lvl = 28,
    .evs = {248, 8, 252, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_DITTO,
    .ball = ITEM_FRIEND_BALL,
    }
};

static const struct TrainerMon sParty_Laura[] = {
    {
    .nickname = _("Caroline"),
    .iv = 200,
    .lvl = 27,
    .evs = {100, 100, 100, 0, 100, 100},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    .heldItem = ITEM_SILK_SCARF,
    .moves = {MOVE_FAKE_OUT, MOVE_SUCKER_PUNCH, MOVE_RETURN, MOVE_THUNDER_WAVE},
    .gender = TRAINER_MON_FEMALE,
    .friendship = TRAINER_MON_FRIENDLY,
    .nature = NATURE_JOLLY,
    .ball = ITEM_FRIEND_BALL,
    },
    {
    .nickname = _("Linus"),
    .iv = 200,
    .lvl = 27,
    .evs = {100, 100, 100, 0, 100, 100},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BELLY_DRUM, MOVE_EXTREME_SPEED, MOVE_STOMPING_TANTRUM, MOVE_SEED_BOMB},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_FRIEND_BALL,
    },
    {
    .nickname = _("Chungus"),
    .iv = 200,
    .lvl = 27,
    .evs = {100, 100, 100, 0, 100, 100},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GREEDENT,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_BODY_SLAM, MOVE_REST, MOVE_MUD_SHOT, MOVE_PSYCHIC_FANGS},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_FRIEND_BALL,
    }
};

static const struct TrainerMon sParty_Cyndy1[] = {
    {
    .iv = 100,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 100,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Cora[] = {
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMon sParty_Paula[] = {
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Cyndy2[] = {
    {
    .iv = 110,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 110,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Cyndy3[] = {
    {
    .iv = 120,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 120,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Cyndy4[] = {
    {
    .iv = 130,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 130,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Cyndy5[] = {
    {
    .iv = 140,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    },
    {
    .iv = 140,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Madeline1[] = {
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    .moves = {MOVE_EMBER, MOVE_TACKLE, MOVE_MAGNITUDE, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMon sParty_Clarissa[] = {
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Angelica[] = {
    {
    .iv = 50,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CASTFORM,
    .moves = {MOVE_RAIN_DANCE, MOVE_WEATHER_BALL, MOVE_THUNDER, MOVE_WATER_PULSE}
    }
};

static const struct TrainerMon sParty_Madeline2[] = {
    {
    .iv = 10,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    .moves = {MOVE_EMBER, MOVE_TACKLE, MOVE_MAGNITUDE, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMon sParty_Madeline3[] = {
    {
    .iv = 20,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    .moves = {MOVE_EMBER, MOVE_TAKE_DOWN, MOVE_MAGNITUDE, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMon sParty_Madeline4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_LEECH_SEED, MOVE_MEGA_DRAIN, MOVE_GRASS_WHISTLE, MOVE_SUNNY_DAY}
    },
    {
    .iv = 30,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    .moves = {MOVE_FLAMETHROWER, MOVE_TAKE_DOWN, MOVE_MAGNITUDE, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMon sParty_Madeline5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_SOLAR_BEAM, MOVE_SUNNY_DAY}
    },
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    .moves = {MOVE_FLAMETHROWER, MOVE_TAKE_DOWN, MOVE_EARTHQUAKE, MOVE_SUNNY_DAY}
    }
};

static const struct TrainerMon sParty_Beverly[] = {
    {
    .nickname = _("Vir"),
    .iv = 100,
    .lvl = 23,
    .evs = {0, 0, 4, 121, 0, 131},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_RAICHU,
    .moves = {MOVE_SWEET_KISS, MOVE_DISCHARGE, MOVE_SURF, MOVE_PROTECT},
    .nature = NATURE_MODEST,
    .ball = ITEM_FRIEND_BALL,
    },
    {
    .nickname = _("Detective"),
    .iv = 100,
    .lvl = 23,
    .evs = {0, 131, 4, 0, 0, 121},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GUMSHOOS,
    .moves = {MOVE_SAND_ATTACK, MOVE_TOXIC, MOVE_DOUBLE_TEAM, MOVE_PAYBACK},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_FRIEND_BALL,
    }
};

static const struct TrainerMon sParty_Imani[] = {
    {
    .nickname = _("Rockstar"),
    .iv = 100,
    .lvl = 23,
    .evs = {0, 121, 4, 0, 0, 131},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_TRANQUILL,
    .moves = {MOVE_ENDEAVOR, MOVE_AERIAL_ACE, MOVE_ROOST, MOVE_STEEL_WING},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_FAST_BALL,
    },
    {
    .nickname = _("Comet"),
    .iv = 100,
    .lvl = 23,
    .evs = {100, 0, 0, 121, 0, 135},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_PIDGEOTTO,
    .moves = {MOVE_TWISTER, MOVE_DOUBLE_TEAM, MOVE_HEAT_WAVE, MOVE_OMINOUS_WIND},
    .nature = NATURE_MODEST,
    .ball = ITEM_SAFARI_BALL,
    }
};

static const struct TrainerMon sParty_Kyla[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Denise[] = {
    {
    .nickname = _("Shoal"),
    .iv = 200,
    .lvl = 26,
    .evs = {252, 0, 0, 252, 0, 4},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VAPOREON,
    .moves = {MOVE_AURORA_BEAM, MOVE_SCALD, MOVE_HELPING_HAND, MOVE_CHARM},
    .nature = NATURE_MODEST,
    .ball = ITEM_PARK_BALL,
    },
    {
    .nickname = _("Sear"),
    .iv = 200,
    .lvl = 27,
    .evs = {4, 252, 0, 252, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_FLAREON,
    .moves = {MOVE_FLAME_WHEEL, MOVE_WEATHER_BALL, MOVE_HELPING_HAND, MOVE_SUPERPOWER},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_PARK_BALL,
    }
};

static const struct TrainerMon sParty_Beth[] = {
    {
    .nickname = _("Leviathan"),
    .iv = 200,
    .lvl = 25,
    .evs = {0, 252, 0, 252, 0, 4},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    .moves = {MOVE_DRAGON_DANCE, MOVE_CRUNCH, MOVE_BULLDOZE, MOVE_DIVE},
    .nature = NATURE_MODEST,
    .ball = ITEM_DIVE_BALL,
    },
    {
    .nickname = _("Crush"),
    .iv = 200,
    .lvl = 27,
    .evs = {252, 0, 4, 0, 252, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TIRTOUGA,
    .moves = {MOVE_ANCIENT_POWER, MOVE_WITHDRAW, MOVE_REST, MOVE_SNORE},
    .nature = NATURE_RELAXED,
    .ball = ITEM_DIVE_BALL,
    },
    {
    .nickname = _("Tad"),
    .iv = 200,
    .lvl = 27,
    .evs = {252, 0, 4, 0, 252, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POLIWHIRL,
    .moves = {MOVE_HYPNOSIS, MOVE_MUD_SHOT, MOVE_BRICK_BREAK, MOVE_MEGA_KICK},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DIVE_BALL,
    }
};

static const struct TrainerMon sParty_Tara[] = {
    {
    .nickname = _("Jupiter"),
    .iv = 200,
    .lvl = 27,
    .evs = {4, 252, 0, 0, 0, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_SKUNTANK,
    .heldItem = ITEM_ROCKY_HELMET,
    .moves = {MOVE_PURSUIT, MOVE_SUCKER_PUNCH, MOVE_POISON_JAB, MOVE_DEFOG},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_PREMIER_BALL,
    },
    {
    .nickname = _("Mars"),
    .iv = 200,
    .lvl = 27,
    .evs = {0, 252, 4, 0, 0, 252},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_PURUGLY,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_FAKE_OUT, MOVE_RETURN, MOVE_KNOCK_OFF, MOVE_BODY_SLAM},
    .gender = TRAINER_MON_FEMALE,
    .friendship = TRAINER_MON_FRIENDLY,
    .nature = NATURE_JOLLY,
    .ball = ITEM_PREMIER_BALL,
    }
};

static const struct TrainerMon sParty_Missy[] = {
    {
    .nickname = _("Hari"),
    .iv = 200,
    .lvl = 30,
    .evs = {252, 252, 4, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_FLAME_ORB,
    .moves = {MOVE_DRAIN_PUNCH, MOVE_MACH_PUNCH, MOVE_KNOCK_OFF, MOVE_FACADE},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname = _("Lycanshi"),
    .iv = 200,
    .lvl = 30,
    .evs = {0, 252, 4, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LYCANROC_DUSK,
    .heldItem = ITEM_ROCKY_HELMET,
    .moves = {MOVE_STEALTH_ROCK, MOVE_FIRE_FANG, MOVE_CRUNCH, MOVE_DRILL_RUN},
    .nature = NATURE_SASSY,
    .ball = ITEM_HEAVY_BALL,
    }
};

static const struct TrainerMon sParty_Alice2[] = {
    {
    .nickname = _("Baby Bob"),
    .iv = 255,
    .lvl = 33,
    .evs = {248, 0, 8, 252, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_EXEGGCUTE,
    .moves = {MOVE_CONFUSION, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_SLUDGE_BOMB},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname = _("Short Sean"),
    .iv = 255,
    .lvl = 33,
    .evs = {248, 0, 8, 252, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_EXEGGUTOR,
    .moves = {MOVE_TRICK_ROOM, MOVE_LEAF_STORM, MOVE_PSYCHIC, MOVE_SLEEP_POWDER},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_QUIET,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname = _("Tall Tim"),
    .iv = 255,
    .lvl = 33,
    .evs = {0, 0, 0, 252, 4, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_EXEGGUTOR_ALOLAN,
    .moves = {MOVE_OUTRAGE, MOVE_LEAF_STORM, MOVE_FLAMETHROWER, MOVE_GIGA_DRAIN},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_GREAT_BALL,
    }
};

static const struct TrainerMon sParty_Jenny1[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Grace[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Tanya[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    }
};

static const struct TrainerMon sParty_Sharon[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMon sParty_Nikki[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPHEAL,
    }
};

static const struct TrainerMon sParty_Brenda[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMon sParty_Katie[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLDEEN,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPHEAL,
    }
};

static const struct TrainerMon sParty_Susie[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    }
};

static const struct TrainerMon sParty_Kara[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMon sParty_Dana[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMon sParty_Sienna[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    }
};

static const struct TrainerMon sParty_Debra[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMon sParty_Linda[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HORSEA,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMon sParty_Kaylee[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    }
};

static const struct TrainerMon sParty_Laurel[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    }
};

static const struct TrainerMon sParty_Carlee[] = {
    {
    .iv = 0,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMon sParty_Jenny2[] = {
    {
    .iv = 0,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Jenny3[] = {
    {
    .iv = 0,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Jenny4[] = {
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    },
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Jenny5[] = {
    {
    .iv = 0,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 0,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 0,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Heidi[] = {
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    .moves = {MOVE_DIG, MOVE_SAND_ATTACK, MOVE_POISON_STING, MOVE_SLASH}
    },
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BALTOY,
    .moves = {MOVE_RAPID_SPIN, MOVE_MUD_SLAP, MOVE_PSYBEAM, MOVE_ROCK_TOMB}
    }
};

static const struct TrainerMon sParty_Becky[] = {
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    .moves = {MOVE_SAND_ATTACK, MOVE_POISON_STING, MOVE_SLASH, MOVE_DIG}
    },
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    .moves = {MOVE_ROLLOUT, MOVE_BUBBLE_BEAM, MOVE_TAIL_WHIP, MOVE_DEFENSE_CURL}
    }
};

static const struct TrainerMon sParty_Carol[] = {
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    }
};

static const struct TrainerMon sParty_Nancy[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    }
};

static const struct TrainerMon sParty_Martha[] = {
    {
    .iv = 0,
    .lvl = 23,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    },
    {
    .iv = 0,
    .lvl = 23,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMon sParty_Diana1[] = {
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ODDISH,
    },
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMon sParty_Cedric[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WOBBUFFET,
    .moves = {MOVE_DESTINY_BOND, MOVE_SAFEGUARD, MOVE_COUNTER, MOVE_MIRROR_COAT}
    }
};

static const struct TrainerMon sParty_Irene[] = {
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Diana2[] = {
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMon sParty_Diana3[] = {
    {
    .iv = 20,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 20,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 20,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMon sParty_Diana4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMon sParty_Diana5[] = {
    {
    .iv = 40,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 40,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VILEPLUME,
    },
    {
    .iv = 40,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    }
};

static const struct TrainerMon sParty_AmyAndLiv1[] = {
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MINUN,
    }
};

static const struct TrainerMon sParty_AmyAndLiv2[] = {
    {
    .iv = 10,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = 10,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MINUN,
    }
};

static const struct TrainerMon sParty_GinaAndMia1[] = {
    {
    .nickname = _("Bruiser"),
    .iv = 100,
    .lvl = 20,
    .evs = {0, 252, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_THROH,
    .moves = {MOVE_CIRCLE_THROW, MOVE_HELPING_HAND, MOVE_PAYBACK, MOVE_ROCK_TOMB},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname = _("Breaker"),
    .iv = 100,
    .lvl = 20,
    .evs = {0, 252, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SAWK,
    .moves = {MOVE_LOW_SWEEP, MOVE_POISON_JAB, MOVE_FIRE_PUNCH, MOVE_HELPING_HAND},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_GREAT_BALL,
    }
};

static const struct TrainerMon sParty_MiuAndYuki[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BEAUTIFLY,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSTOX,
    }
};

static const struct TrainerMon sParty_AmyAndLiv3[] = {
    {
    .iv = 0,
    .lvl = 9,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = 0,
    .lvl = 9,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MINUN,
    }
};

static const struct TrainerMon sParty_GinaAndMia2[] = {
    {
    .iv = 0,
    .lvl = 10,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSKULL,
    .moves = {MOVE_NIGHT_SHADE, MOVE_DISABLE, MOVE_NONE, MOVE_NONE}
    },
    {
    .iv = 0,
    .lvl = 10,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    .moves = {MOVE_ABSORB, MOVE_LEECH_SEED, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_AmyAndLiv4[] = {
    {
    .iv = 20,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PLUSLE,
    },
    {
    .iv = 20,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MINUN,
    }
};

static const struct TrainerMon sParty_AmyAndLiv5[] = {
    {
    .iv = 30,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PLUSLE,
    .moves = {MOVE_SPARK, MOVE_CHARGE, MOVE_FAKE_TEARS, MOVE_HELPING_HAND}
    },
    {
    .iv = 30,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MINUN,
    .moves = {MOVE_SPARK, MOVE_CHARGE, MOVE_CHARM, MOVE_HELPING_HAND}
    }
};

static const struct TrainerMon sParty_AmyAndLiv6[] = {
    {
    .iv = 40,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PLUSLE,
    .moves = {MOVE_THUNDER, MOVE_CHARGE, MOVE_FAKE_TEARS, MOVE_HELPING_HAND}
    },
    {
    .iv = 40,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MINUN,
    .moves = {MOVE_THUNDER, MOVE_CHARGE, MOVE_CHARM, MOVE_HELPING_HAND}
    }
};

static const struct TrainerMon sParty_Huey[] = {
    {
    .nickname = _("Penelipper"),
    .iv = 255,
    .lvl = 33,
    .evs = {248, 0, 36, 0, 224, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_PELIPPER,
    .moves = {MOVE_PROTECT, MOVE_SCALD, MOVE_AIR_SLASH, MOVE_MUD_SLAP},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_RELAXED,
    .ball = ITEM_DIVE_BALL,
    .shiny = TRUE,
    },
    {
    .nickname = _("Polly"),
    .iv = 255,
    .lvl = 34,
    .evs = {248, 0, 0, 0, 252, 8},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_POLITOED,
    .moves = {MOVE_EARTHQUAKE, MOVE_BODY_SLAM, MOVE_ICE_PUNCH, MOVE_SURF},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DIVE_BALL,
    }
};

static const struct TrainerMon sParty_Edmond[] = {
    {
    .nickname = _("Lucy"),
    .iv = 255,
    .lvl = 34,
    .evs = {0, 252, 0, 0, 4, 252},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_LUXRAY,
    .heldItem = ITEM_FLAME_ORB,
    .moves = {MOVE_WILD_CHARGE, MOVE_FACADE, MOVE_SUPERPOWER, MOVE_CRUNCH},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Skullivan"),
    .iv = 255,
    .lvl = 34,
    .evs = {196, 0, 196, 0, 116, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSKULL,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_PAIN_SPLIT, MOVE_WILL_O_WISP, MOVE_SUBSTITUTE, MOVE_HEX},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_CAREFUL,
    .ball = ITEM_DUSK_BALL,
    }
};

static const struct TrainerMon sParty_Ernest1[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Dwayne[] = {
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMon sParty_Phillip[] = {
    {
    .iv = 0,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    },
    {
    .iv = 0,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Leonard[] = {
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Duncan[] = {
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPHEAL,
    },
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Ernest2[] = {
    {
    .iv = 10,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 10,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 10,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Ernest3[] = {
    {
    .iv = 20,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 20,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 20,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Ernest4[] = {
    {
    .iv = 30,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 30,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 30,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Ernest5[] = {
    {
    .iv = 40,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 40,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 40,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Eli[] = {
    {
    .iv = 100,
    .lvl = 23,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Annika[] = {
    {
    .iv = 100,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_FEEBAS,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_FLAIL, MOVE_WATER_PULSE, MOVE_RETURN, MOVE_ATTRACT}
    },
    {
    .iv = 100,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_FEEBAS,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_FLAIL, MOVE_WATER_PULSE, MOVE_RETURN, MOVE_ATTRACT}
    }
};

static const struct TrainerMon sParty_Jazmyn[] = {
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ABSOL,
    }
};

static const struct TrainerMon sParty_Jonas[] = {
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_TOXIC, MOVE_THUNDER, MOVE_SELF_DESTRUCT, MOVE_SLUDGE_BOMB}
    }
};

static const struct TrainerMon sParty_Kayley[] = {
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CASTFORM,
    .moves = {MOVE_SUNNY_DAY, MOVE_WEATHER_BALL, MOVE_FLAMETHROWER, MOVE_SOLAR_BEAM}
    }
};

static const struct TrainerMon sParty_Auron[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMon sParty_Kelvin[] = {
    {
    .iv = 150,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 150,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPHEAL,
    }
};

static const struct TrainerMon sParty_Marley[] = {
    {
    .iv = 255,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_BITE, MOVE_ROAR, MOVE_THUNDER_WAVE, MOVE_THUNDERBOLT}
    }
};

static const struct TrainerMon sParty_Reyna[] = {
    {
    .iv = 50,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 200,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Hudson[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Conor[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CHINCHOU,
    },
    {
    .iv = 200,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Edwin1[] = {
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Hector[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZANGOOSE,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEVIPER,
    }
};

static const struct TrainerMon sParty_TabithaMossdeep[] = {
    {
    .iv = 100,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    },
    {
    .iv = 100,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 100,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLBAT,
    }
};

static const struct TrainerMon sParty_Edwin2[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Edwin3[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Edwin4[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Edwin5[] = {
    {
    .iv = 0,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 0,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHIFTRY,
    }
};

static const struct TrainerMon sParty_WallyVR1[] = {
    {
    .iv = 150,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .moves = {MOVE_AERIAL_ACE, MOVE_SAFEGUARD, MOVE_DRAGON_BREATH, MOVE_DRAGON_DANCE}
    },
    {
    .iv = 150,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    .moves = {MOVE_SING, MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 150,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_MAGICAL_LEAF, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_TOXIC}
    },
    {
    .iv = 150,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SUPERSONIC, MOVE_THUNDERBOLT, MOVE_TRI_ATTACK, MOVE_SCREECH}
    },
    {
    .iv = 250,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GARDEVOIR,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_CALM_MIND, MOVE_PSYCHIC, MOVE_FUTURE_SIGHT}
    }
};

static const struct TrainerMon sParty_BrendanRoute103Mudkip[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TREECKO,
    }
};

static const struct TrainerMon sParty_BrendanRoute110Mudkip[] = {
    {
    .iv = 50,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 50,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 100,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GROVYLE,
    }
};

static const struct TrainerMon sParty_BrendanRoute119Mudkip[] = {
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 150,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GROVYLE,
    }
};

static const struct TrainerMon sParty_BrendanRoute103Treecko[] = {
    {
    .nickname = _("Leon"),
    .iv = 0,
    .lvl = 6,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WEEDLE,
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_CHERISH_BALL,
    }
};

static const struct TrainerMon sParty_BrendanRoute110Treecko[] = {
{
    .nickname = _("Allister"),
    .iv = 255,
    .lvl = 31,
    .evs = {4, 0, 0, 252, 0, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_LITWICK,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_SHADOW_BALL, MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_CONFUSE_RAY},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname = _("Bede"),
    .iv = 255,
    .lvl = 32,
    .evs = {188, 0, 0, 192, 0, 128},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MORGREM,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_REFLECT, MOVE_THUNDER_WAVE, MOVE_DAZZLING_GLEAM},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("Nessa"),
    .iv = 255,
    .lvl = 31,
    .evs = {0, 0, 0, 252, 136, 120},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_ASSAULT_VEST,
    .moves = {MOVE_SCALD, MOVE_VOLT_SWITCH, MOVE_ICE_BEAM, MOVE_DISCHARGE},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_DIVE_BALL,
    },
    {
    .nickname = _("Piers"),
    .iv = 255,
    .lvl = 33,
    .evs = {36, 196, 196, 0, 36, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRIMER_ALOLAN,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_PURSUIT, MOVE_KNOCK_OFF, MOVE_POISON_JAB, MOVE_SHADOW_SNEAK},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("Leon"),
    .iv = 255,
    .lvl = 34,
    .evs = {0, 252, 0, 0, 4, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BEEDRILL,
    .heldItem = ITEM_BEEDRILLITE,
    .moves = {MOVE_DRILL_RUN, MOVE_POISON_JAB, MOVE_PROTECT, MOVE_PURSUIT},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_CHERISH_BALL,
    }
};

static const struct TrainerMon sParty_BrendanRoute119Treecko[] = {
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 150,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_COMBUSKEN,
    }
};

static const struct TrainerMon sParty_BrendanRoute103Torchic[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MUDKIP,
    }
};

static const struct TrainerMon sParty_BrendanRoute110Torchic[] = {
    {
    .iv = 50,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 50,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 100,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARSHTOMP,
    }
};

static const struct TrainerMon sParty_BrendanRoute119Torchic[] = {
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 150,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARSHTOMP,
    }
};

static const struct TrainerMon sParty_MayRoute103Mudkip[] = {
    {
    .nickname = _("Sandshine"),
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    .moves = {MOVE_SCRATCH, MOVE_DEFENSE_CURL, MOVE_POISON_STING, MOVE_SAND_ATTACK},
    .nature = NATURE_BRAVE,
    .ball = ITEM_PARK_BALL,
    .shiny = TRUE,
    },
    {
    .nickname = _("Funguy"),
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PARAS,
    .moves = {MOVE_SCRATCH, MOVE_SLEEP_POWDER, MOVE_DIG, MOVE_STUN_SPORE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_PARK_BALL,
    }
};

static const struct TrainerMon sParty_MayRoute110Mudkip[] = {
    {
    .iv = 50,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 50,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 100,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GROVYLE,
    }
};

static const struct TrainerMon sParty_MayRoute119Mudkip[] = {
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 150,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GROVYLE,
    }
};

static const struct TrainerMon sParty_MayRoute103Treecko[] = {
    {
    .nickname = _("Leon"),
    .iv = 0,
    .lvl = 6,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WEEDLE,
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_CHERISH_BALL,
    }
};

static const struct TrainerMon sParty_MayRoute110Treecko[] = {
{
    .nickname = _("Allister"),
    .iv = 255,
    .lvl = 31,
    .evs = {4, 0, 0, 252, 0, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_LITWICK,
    .heldItem = ITEM_EXPERT_BELT,
    .moves = {MOVE_SHADOW_BALL, MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_CONFUSE_RAY},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname = _("Bede"),
    .iv = 255,
    .lvl = 32,
    .evs = {188, 0, 0, 192, 0, 128},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MORGREM,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_REFLECT, MOVE_THUNDER_WAVE, MOVE_DAZZLING_GLEAM},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("Nessa"),
    .iv = 255,
    .lvl = 31,
    .evs = {0, 0, 0, 252, 136, 120},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LANTURN,
    .heldItem = ITEM_ASSAULT_VEST,
    .moves = {MOVE_SCALD, MOVE_VOLT_SWITCH, MOVE_ICE_BEAM, MOVE_DISCHARGE},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_DIVE_BALL,
    },
    {
    .nickname = _("Piers"),
    .iv = 255,
    .lvl = 33,
    .evs = {36, 196, 196, 0, 36, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRIMER_ALOLAN,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_PURSUIT, MOVE_KNOCK_OFF, MOVE_POISON_JAB, MOVE_SHADOW_SNEAK},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("Leon"),
    .iv = 255,
    .lvl = 34,
    .evs = {0, 252, 0, 0, 4, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BEEDRILL,
    .heldItem = ITEM_BEEDRILLITE,
    .moves = {MOVE_DRILL_RUN, MOVE_POISON_JAB, MOVE_PROTECT, MOVE_PURSUIT},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_CHERISH_BALL,
    }
};

static const struct TrainerMon sParty_MayRoute119Treecko[] = {
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 150,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_COMBUSKEN,
    }
};

static const struct TrainerMon sParty_MayRoute103Torchic[] = {
    {
    .nickname = _("Prophet"),
    .iv = 100,
    .lvl = 9,
    .evs = {252, 0, 252, 4, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_NATU,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_STORED_POWER, MOVE_NONE, MOVE_LEER, MOVE_CALM_MIND},
    .nature = NATURE_HASTY,
    .ball = ITEM_TIMER_BALL,
    },
    {
    .nickname = _("Reaper"),
    .iv = 200,
    .lvl = 10,
    .evs = {0, 252, 0, 0, 4, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_MURKROW,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_NONE, MOVE_WING_ATTACK, MOVE_CONFUSE_RAY, MOVE_ROOST},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DUSK_BALL,
    }
};

static const struct TrainerMon sParty_MayRoute110Torchic[] = {
    {
    .nickname = _("Prophet"),
    .iv = 155,
    .lvl = 30,
    .evs = {252, 0, 252, 4, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_XATU,
    .heldItem = ITEM_ROCKY_HELMET,
    .moves = {MOVE_PSYCHIC, MOVE_NONE, MOVE_PROTECT, MOVE_THUNDER_WAVE},
    .nature = NATURE_TIMID,
    .ball = ITEM_TIMER_BALL,
    },
    {
    .nickname = _("Apophis"),
    .iv = 255,
    .lvl = 30,
    .evs = {0, 252, 4, 0, 0, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_PIDGEOTTO,
    .heldItem = ITEM_SKY_PLATE,
    .moves = {MOVE_NONE, MOVE_DEFOG, MOVE_SAND_ATTACK, MOVE_ROOST},
    .nature = NATURE_JOLLY,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Reaper"),
    .iv = 255,
    .lvl = 31,
    .evs = {0, 252, 252, 0, 4, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_MURKROW,
    .heldItem = ITEM_ORAN_BERRY,
    .moves = {MOVE_NONE, MOVE_DOUBLE_TEAM, MOVE_ROOST, MOVE_SUBSTITUTE},
    .nature = NATURE_JOLLY,
    .ball = ITEM_DUSK_BALL,
    }
};

static const struct TrainerMon sParty_MayRoute119Torchic[] = {
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 150,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARSHTOMP,
    }
};

static const struct TrainerMon sParty_Isaac1[] = {
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WHISMUR,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARON,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Davis[] = {
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PINSIR,
    }
};

static const struct TrainerMon sParty_Mitchell[] = {
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUNATONE,
    .moves = {MOVE_EXPLOSION, MOVE_REFLECT, MOVE_LIGHT_SCREEN, MOVE_PSYCHIC}
    },
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SOLROCK,
    .moves = {MOVE_EXPLOSION, MOVE_REFLECT, MOVE_LIGHT_SCREEN, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMon sParty_Isaac2[] = {
    {
    .iv = 10,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 10,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 10,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARON,
    },
    {
    .iv = 10,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 10,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 10,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Isaac3[] = {
    {
    .iv = 20,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 20,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 20,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARON,
    },
    {
    .iv = 20,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 20,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 20,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Isaac4[] = {
    {
    .iv = 30,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 30,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 30,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARON,
    },
    {
    .iv = 30,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 30,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 30,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Isaac5[] = {
    {
    .iv = 40,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 40,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 40,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LAIRON,
    },
    {
    .iv = 40,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 40,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 40,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    }
};

static const struct TrainerMon sParty_Lydia1[] = {
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    },
    {
    .iv = 0,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMon sParty_Halle[] = {
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SABLEYE,
    },
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ABSOL,
    }
};

static const struct TrainerMon sParty_Garrison[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSLASH,
    }
};

static const struct TrainerMon sParty_Lydia2[] = {
    {
    .iv = 10,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 10,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 10,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 10,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 10,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    },
    {
    .iv = 10,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMon sParty_Lydia3[] = {
    {
    .iv = 20,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 20,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 20,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 20,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 20,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 20,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMon sParty_Lydia4[] = {
    {
    .iv = 30,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 30,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 30,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 30,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 30,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 30,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMon sParty_Lydia5[] = {
    {
    .iv = 40,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 40,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    },
    {
    .iv = 40,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 40,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 40,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 40,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEAKING,
    }
};

static const struct TrainerMon sParty_Jackson1[] = {
    {
    .iv = 50,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Lorenzo[] = {
    {
    .iv = 50,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEEDOT,
    },
    {
    .iv = 50,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUZLEAF,
    },
    {
    .iv = 50,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    }
};

static const struct TrainerMon sParty_Sebastian[] = {
    {
    .iv = 50,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CACTURNE,
    }
};

static const struct TrainerMon sParty_Jackson2[] = {
    {
    .iv = 60,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Jackson3[] = {
    {
    .iv = 70,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Jackson4[] = {
    {
    .iv = 80,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Jackson5[] = {
    {
    .iv = 90,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KECLEON,
    },
    {
    .iv = 90,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Catherine1[] = {
    {
    .iv = 50,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 50,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Jenna[] = {
    {
    .iv = 50,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOTAD,
    },
    {
    .iv = 50,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 50,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Sophia[] = {
    {
    .iv = 50,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    },
    {
    .iv = 50,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Catherine2[] = {
    {
    .iv = 60,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 60,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Catherine3[] = {
    {
    .iv = 70,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 70,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Catherine4[] = {
    {
    .iv = 80,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 80,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Catherine5[] = {
    {
    .iv = 90,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BELLOSSOM,
    },
    {
    .iv = 90,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Julio[] = {
    {
    .iv = 0,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_GruntSeafloorCavern5[] = {
    {
    .iv = 50,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 50,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLBAT,
    }
};

static const struct TrainerMon sParty_GruntUnused[] = {
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_GruntMtPyre4[] = {
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_GruntJaggedPass[] = {
    {
    .iv = 50,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 50,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Marc[] = {
    {
    .nickname = _("Banshee"),
    .iv = 120,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MISDREAVUS,
    .moves = {MOVE_PSYWAVE, MOVE_CONFUSE_RAY, MOVE_DAZZLING_GLEAM, MOVE_SHADOW_BALL},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Helium"),
    .iv = 130,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DRIFLOON,
    .moves = {MOVE_GUST, MOVE_PAYBACK, MOVE_WILL_O_WISP, MOVE_PROTECT},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DUSK_BALL,
    }
};

static const struct TrainerMon sParty_Brenden[] = {
    {
    .nickname = _("Benedict"),
    .iv = 200,
    .lvl = 28,
    .evs = {252, 0, 252, 0, 4, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CHANSEY,
    .heldItem = ITEM_EVIOLITE,
    .moves = {MOVE_SEISMIC_TOSS, MOVE_TOXIC, MOVE_SOFT_BOILED, MOVE_STEALTH_ROCK},
    .nature = NATURE_BOLD,
    .ball = ITEM_FRIEND_BALL,
    },
    {
    .nickname = _("Zodiac"),
    .iv = 200,
    .lvl = 29,
    .evs = {0, 252, 4, 0, 0, 252},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_TAUROS,
    .heldItem = ITEM_LIFE_ORB,
    .moves = {MOVE_BODY_SLAM, MOVE_CLOSE_COMBAT, MOVE_THROAT_CHOP, MOVE_IRON_HEAD},
    .nature = NATURE_JOLLY,
    .ball = ITEM_FRIEND_BALL,
    }
};

static const struct TrainerMon sParty_Lilith[] = {
    {
    .nickname = _("Dunce"),
    .iv = 200,
    .lvl = 29,
    .evs = {100, 100, 100, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUNSPARCE,
    .heldItem = ITEM_ROCKY_HELMET,
    .moves = {MOVE_DRAGON_RAGE, MOVE_DRAGON_PULSE, MOVE_PROTECT, MOVE_REST},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_FRIEND_BALL,
    },
    {
    .nickname = _("Whitney"),
    .iv = 200,
    .lvl = 29,
    .evs = {100, 100, 100, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MILTANK,
    .moves = {MOVE_STOMP, MOVE_ATTRACT, MOVE_MILK_DRINK, MOVE_ROLLOUT},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_FRIEND_BALL,
    }
};

static const struct TrainerMon sParty_Cristian[] = {
    {
    .nickname = _("Harambe"),
    .iv = 200,
    .lvl = 29,
    .evs = {248, 0, 0, 252, 8, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ORANGURU,
    .heldItem = ITEM_COLBUR_BERRY,
    .moves = {MOVE_TRICK_ROOM, MOVE_NASTY_PLOT, MOVE_PSYCHIC, MOVE_FOCUS_BLAST},
    .nature = NATURE_QUIET,
    .ball = ITEM_FRIEND_BALL,
    },
    {
    .nickname = _("Audiyes"),
    .iv = 200,
    .lvl = 29,
    .evs = {252, 0, 4, 0, 252, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AUDINO,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_WISH, MOVE_PROTECT, MOVE_HEAL_BELL, MOVE_KNOCK_OFF},
    .nature = NATURE_CALM,
    .ball = ITEM_FRIEND_BALL,
    }
};

static const struct TrainerMon sParty_Sylvia[] = {
    {
    .iv = 100,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMon sParty_Leonardo[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_Athena[] = {
    {
    .iv = 100,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_THUNDER, MOVE_THUNDER_WAVE, MOVE_QUICK_ATTACK, MOVE_NONE}
    },
    {
    .iv = 100,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_THIEF, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Harrison[] = {
    {
    .iv = 0,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_GruntMtChimney2[] = {
    {
    .iv = 0,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_Clarence[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Terry[] = {
    {
    .iv = 0,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GIRAFARIG,
    }
};

static const struct TrainerMon sParty_Nate[] = {
    {
    .iv = 100,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPOINK,
    }
};

static const struct TrainerMon sParty_Kathleen[] = {
    {
    .iv = 100,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    }
};

static const struct TrainerMon sParty_Clifford[] = {
    {
    .iv = 0,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GIRAFARIG,
    }
};

static const struct TrainerMon sParty_Nicholas[] = {
    {
    .iv = 0,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WOBBUFFET,
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter3[] = {
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    },
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter4[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BALTOY,
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter5[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter6[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMon sParty_GruntSpaceCenter7[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BALTOY,
    }
};

static const struct TrainerMon sParty_Macey[] = {
    {
    .iv = 0,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NATU,
    }
};

static const struct TrainerMon sParty_BrendanRustboroTreecko[] = {
{
    .nickname = _("Allister"),
    .iv = 255,
    .lvl = 15,
    .evs = {4, 0, 0, 252, 0, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_LITWICK,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HEX, MOVE_WILL_O_WISP, MOVE_CONFUSE_RAY, MOVE_SUBSTITUTE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname =_("Nessa"),
    .iv = 255,
    .lvl = 16,
    .evs = {52, 252, 0, 0, 0, 204},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CHINCHOU,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_BUBBLE_BEAM, MOVE_THUNDER_WAVE, MOVE_ELECTRO_BALL, MOVE_SUPERSONIC},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_DIVE_BALL,
    },
    {
    .nickname = _("Leon"),
    .iv = 255,
    .lvl = 15,
    .evs = {0, 252, 4, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BEEDRILL,
    .heldItem = ITEM_BEEDRILLITE,
    .moves = {MOVE_DRILL_RUN, MOVE_FURY_ATTACK, MOVE_POISON_STING, MOVE_PROTECT},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_CHERISH_BALL,
    }
};

static const struct TrainerMon sParty_BrendanRustboroMudkip[] = {
    {
    .iv = 25,
    .lvl = 13,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 50,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TREECKO,
    }
};

static const struct TrainerMon sParty_Paxton[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Isabella[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_GruntWeatherInst5[] = {
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    },
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    }
};

static const struct TrainerMon sParty_TabithaMtChimney[] = {
    {
    .iv = 50,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 50,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 50,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 50,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_Jonathan[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KECLEON,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_BrendanRustboroTorchic[] = {
    {
    .iv = 25,
    .lvl = 13,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 50,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MUDKIP,
    }
};

static const struct TrainerMon sParty_MayRustboroMudkip[] = {
    {
    .iv = 25,
    .lvl = 13,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 50,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TREECKO,
    }
};

static const struct TrainerMon sParty_MaxieMagmaHideout[] = {
    {
    .iv = 150,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 150,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CROBAT,
    },
    {
    .iv = 150,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMon sParty_MaxieMtChimney[] = {
    {
    .iv = 150,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 150,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    },
    {
    .iv = 150,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMon sParty_Tiana[] = {
    {
    .iv = 0,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 0,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    }
};

static const struct TrainerMon sParty_Haley1[] = {
    {
    .nickname = _("Snowy"),
    .iv = 100,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_SNOVER,
    .moves = {MOVE_ICE_SHARD, MOVE_MAGICAL_LEAF, MOVE_WATER_PULSE, MOVE_MEGA_PUNCH},
    .nature = NATURE_HASTY,
    .ball = ITEM_HEAL_BALL,
    },
    {
    .nickname = _("Smoochy"),
    .iv = 100,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_SMOOCHUM,
    .moves = {MOVE_CONFUSION, MOVE_DRAINING_KISS, MOVE_METRONOME, MOVE_NASTY_PLOT},
    .nature = NATURE_MODEST,
    .ball = ITEM_HEAL_BALL,
    }
};

static const struct TrainerMon sParty_Janice[] = {
    {
    .iv = 0,
    .lvl = 9,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Vivi[] = {
    {
    .iv = 100,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 100,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 100,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Haley2[] = {
    {
    .iv = 10,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 10,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    }
};

static const struct TrainerMon sParty_Haley3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Haley4[] = {
    {
    .iv = 30,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 30,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Haley5[] = {
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 40,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BRELOOM,
    }
};

static const struct TrainerMon sParty_Sally[] = {
    {
    .iv = 0,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ODDISH,
    }
};

static const struct TrainerMon sParty_Robin[] = {
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    },
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Andrea[] = {
    {
    .iv = 100,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    }
};

static const struct TrainerMon sParty_Crissy[] = {
    {
    .iv = 100,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLDEEN,
    },
    {
    .iv = 100,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Rick[] = {
    {
    .iv = 0,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WURMPLE,
    },
    {
    .iv = 0,
    .lvl = 4,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WURMPLE,
    }
};

static const struct TrainerMon sParty_Lyle[] = {
    {
    .nickname = _("Spike"),
    .iv = 200,
    .lvl = 24,
    .evs = {0, 100, 100, 0, 100, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RHYHORN,
    .moves = {MOVE_ROCK_SLIDE, MOVE_MUD_SHOT, MOVE_FIRE_FANG, MOVE_THUNDERBOLT},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Scarecrow"),
    .iv = 200,
    .lvl = 24,
    .evs = {0, 200, 0, 0, 0, 75},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_FEAROW,
    .moves = {MOVE_ASSURANCE, MOVE_ROOST, MOVE_DRILL_RUN, MOVE_STEEL_WING},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_ULTRA_BALL,
    },
};

static const struct TrainerMon sParty_Jose[] = {
    {
    .iv = 50,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WURMPLE,
    },
    {
    .iv = 50,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINCADA,
    }
};

static const struct TrainerMon sParty_Doug[] = {
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINCADA,
    },
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_Greg[] = {
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMon sParty_Kent[] = {
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_James1[] = {
    {
    .nickname = _("Nyx"),
    .iv = 200,
    .lvl = 24,
    .evs = {0, 75, 0, 100, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HOUNDOOM,
    .moves = {MOVE_SMOG, MOVE_FLAMETHROWER, MOVE_EMBARGO, MOVE_DARK_PULSE},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Penumbra"),
    .iv = 200,
    .lvl = 25,
    .evs = {0, 0, 0, 100, 0, 75},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HAUNTER,
    .moves = {MOVE_HYPNOSIS, MOVE_DAZZLING_GLEAM, MOVE_SLUDGE_WAVE, MOVE_SHADOW_BALL},
    .nature = NATURE_MODEST,
    .ball = ITEM_ULTRA_BALL,
    }
};

static const struct TrainerMon sParty_James2[] = {
    {
    .iv = 10,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_James3[] = {
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = 20,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_James4[] = {
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = 30,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_James5[] = {
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SURSKIT,
    },
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINJASK,
    },
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINJASK,
    }
};

static const struct TrainerMon sParty_Brice[] = {
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMon sParty_Trent1[] = {
    {
    .iv = 0,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 0,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    }
};

static const struct TrainerMon sParty_Lenny[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMon sParty_Lucas1[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Alan[] = {
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NOSEPASS,
    },
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    }
};

static const struct TrainerMon sParty_Clark[] = {
    {
    .iv = 0,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    }
};

static const struct TrainerMon sParty_Eric[] = {
    {
    .iv = 0,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 0,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BALTOY,
    }
};

static const struct TrainerMon sParty_Lucas2[] = {
    {
    .iv = 0,
    .lvl = 9,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    .moves = {MOVE_SPLASH, MOVE_WATER_GUN, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Mike1[] = {
    {
    .iv = 0,
    .lvl = 10,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    .moves = {MOVE_GUST, MOVE_GROWL, MOVE_NONE, MOVE_NONE}
    },
    {
    .iv = 0,
    .lvl = 10,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    .moves = {MOVE_BITE, MOVE_SCARY_FACE, MOVE_NONE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Mike2[] = {
    {
    .iv = 0,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 0,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 0,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    }
};

static const struct TrainerMon sParty_Trent2[] = {
    {
    .iv = 10,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 10,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 10,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 10,
    .lvl = 24,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    }
};

static const struct TrainerMon sParty_Trent3[] = {
    {
    .iv = 20,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 20,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 20,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 20,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    }
};

static const struct TrainerMon sParty_Trent4[] = {
    {
    .iv = 30,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 30,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 30,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 30,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    }
};

static const struct TrainerMon sParty_Trent5[] = {
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    },
    {
    .iv = 40,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLEM,
    }
};

static const struct TrainerMon sParty_DezAndLuke[] = {
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMon sParty_LeaAndJed[] = {
    {
    .iv = 0,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 0,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    }
};

static const struct TrainerMon sParty_KiraAndDan1[] = {
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMon sParty_KiraAndDan2[] = {
    {
    .iv = 10,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 10,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMon sParty_KiraAndDan3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMon sParty_KiraAndDan4[] = {
    {
    .iv = 30,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 30,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMon sParty_KiraAndDan5[] = {
    {
    .iv = 40,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VOLBEAT,
    },
    {
    .iv = 40,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ILLUMISE,
    }
};

static const struct TrainerMon sParty_Johanna[] = {
    {
    .iv = 0,
    .lvl = 13,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMon sParty_Gerald[] = {
    {
    .iv = 100,
    .lvl = 23,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KECLEON,
    .moves = {MOVE_FLAMETHROWER, MOVE_FURY_SWIPES, MOVE_FEINT_ATTACK, MOVE_BIND}
    }
};

static const struct TrainerMon sParty_Vivian[] = {
    {
    .nickname = _("Excalibird"),
    .iv = 100,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_SKARMORY,
    .moves = {MOVE_AIR_SLASH, MOVE_STEEL_WING, MOVE_ROCK_TOMB, MOVE_SAND_TOMB},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_HEAVY_BALL,
    },
    {
    .nickname = _("Olive Ore"),
    .iv = 100,
    .lvl = 21,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_BRONZOR,
    .moves = {MOVE_CONFUSE_RAY, MOVE_IMPRISON, MOVE_PAYBACK, MOVE_GYRO_BALL}
    }
};

static const struct TrainerMon sParty_Danielle[] = {
    {
    .iv = 100,
    .lvl = 23,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    .moves = {MOVE_BIDE, MOVE_DETECT, MOVE_CONFUSION, MOVE_FIRE_PUNCH}
    }
};

static const struct TrainerMon sParty_Hideo[] = {
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_TACKLE, MOVE_SELF_DESTRUCT, MOVE_SLUDGE, MOVE_SMOKESCREEN}
    },
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_TACKLE, MOVE_POISON_GAS, MOVE_SLUDGE, MOVE_SMOKESCREEN}
    }
};

static const struct TrainerMon sParty_Keigo[] = {
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_SELF_DESTRUCT, MOVE_SLUDGE, MOVE_SMOKESCREEN}
    },
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINJASK,
    .moves = {MOVE_SAND_ATTACK, MOVE_DOUBLE_TEAM, MOVE_FURY_CUTTER, MOVE_SWORDS_DANCE}
    }
};

static const struct TrainerMon sParty_Riley[] = {
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINCADA,
    .moves = {MOVE_LEECH_LIFE, MOVE_FURY_SWIPES, MOVE_MIND_READER, MOVE_DIG}
    },
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_TACKLE, MOVE_SELF_DESTRUCT, MOVE_SLUDGE, MOVE_SMOKESCREEN}
    }
};

static const struct TrainerMon sParty_Flint[] = {
    {
    .iv = 150,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 150,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_XATU,
    }
};

static const struct TrainerMon sParty_Ashley[] = {
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    },
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    },
    {
    .iv = 100,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    }
};

static const struct TrainerMon sParty_WallyMauville[] = {
    {
    .iv = 30,
    .lvl = 16,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RALTS,
    }
};

static const struct TrainerMon sParty_WallyVR2[] = {
    {
    .iv = 150,
    .lvl = 47,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .moves = {MOVE_AERIAL_ACE, MOVE_SAFEGUARD, MOVE_DRAGON_BREATH, MOVE_DRAGON_DANCE}
    },
    {
    .iv = 150,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    .moves = {MOVE_SING, MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 150,
    .lvl = 47,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_MAGICAL_LEAF, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_TOXIC}
    },
    {
    .iv = 150,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SUPERSONIC, MOVE_THUNDERBOLT, MOVE_TRI_ATTACK, MOVE_SCREECH}
    },
    {
    .iv = 250,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GARDEVOIR,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_CALM_MIND, MOVE_PSYCHIC, MOVE_FUTURE_SIGHT}
    }
};

static const struct TrainerMon sParty_WallyVR3[] = {
    {
    .iv = 150,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .moves = {MOVE_AERIAL_ACE, MOVE_SAFEGUARD, MOVE_DRAGON_BREATH, MOVE_DRAGON_DANCE}
    },
    {
    .iv = 150,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    .moves = {MOVE_SING, MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 150,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_MAGICAL_LEAF, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_TOXIC}
    },
    {
    .iv = 150,
    .lvl = 47,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SUPERSONIC, MOVE_THUNDERBOLT, MOVE_TRI_ATTACK, MOVE_SCREECH}
    },
    {
    .iv = 250,
    .lvl = 51,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GARDEVOIR,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_CALM_MIND, MOVE_PSYCHIC, MOVE_FUTURE_SIGHT}
    }
};

static const struct TrainerMon sParty_WallyVR4[] = {
    {
    .iv = 150,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .moves = {MOVE_AERIAL_ACE, MOVE_SAFEGUARD, MOVE_DRAGON_BREATH, MOVE_DRAGON_DANCE}
    },
    {
    .iv = 150,
    .lvl = 52,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    .moves = {MOVE_SING, MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 150,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_MAGICAL_LEAF, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_TOXIC}
    },
    {
    .iv = 150,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SUPERSONIC, MOVE_THUNDERBOLT, MOVE_TRI_ATTACK, MOVE_SCREECH}
    },
    {
    .iv = 250,
    .lvl = 54,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GARDEVOIR,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_CALM_MIND, MOVE_PSYCHIC, MOVE_FUTURE_SIGHT}
    }
};

static const struct TrainerMon sParty_WallyVR5[] = {
    {
    .iv = 150,
    .lvl = 56,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .moves = {MOVE_AERIAL_ACE, MOVE_SAFEGUARD, MOVE_DRAGON_BREATH, MOVE_DRAGON_DANCE}
    },
    {
    .iv = 150,
    .lvl = 55,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    .moves = {MOVE_SING, MOVE_ASSIST, MOVE_CHARM, MOVE_FEINT_ATTACK}
    },
    {
    .iv = 150,
    .lvl = 56,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_MAGICAL_LEAF, MOVE_LEECH_SEED, MOVE_GIGA_DRAIN, MOVE_TOXIC}
    },
    {
    .iv = 150,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    .moves = {MOVE_SUPERSONIC, MOVE_THUNDERBOLT, MOVE_TRI_ATTACK, MOVE_SCREECH}
    },
    {
    .iv = 250,
    .lvl = 57,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GARDEVOIR,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_CALM_MIND, MOVE_PSYCHIC, MOVE_FUTURE_SIGHT}
    }
};

static const struct TrainerMon sParty_BrendanLilycoveMudkip[] = {
    {
    .iv = 150,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 150,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 150,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 200,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GROVYLE,
    }
};

static const struct TrainerMon sParty_BrendanLilycoveTreecko[] = {
    {
    .iv = 150,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 150,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 150,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 200,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_COMBUSKEN,
    }
};

static const struct TrainerMon sParty_BrendanLilycoveTorchic[] = {
    {
    .iv = 150,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 150,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 150,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 200,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARSHTOMP,
    }
};

static const struct TrainerMon sParty_MayLilycoveMudkip[] = {
    {
    .iv = 150,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 150,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 150,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 200,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GROVYLE,
    }
};

static const struct TrainerMon sParty_MayLilycoveTreecko[] = {
    {
    .iv = 150,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 150,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 150,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 200,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_COMBUSKEN,
    }
};

static const struct TrainerMon sParty_MayLilycoveTorchic[] = {
    {
    .iv = 150,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 150,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 150,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 200,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARSHTOMP,
    }
};

static const struct TrainerMon sParty_Jonah[] = {
    {
    .iv = 0,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Henry[] = {
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    },
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Roger[] = {
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGIKARP,
    },
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGIKARP,
    },
    {
    .iv = 0,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GYARADOS,
    }
};

static const struct TrainerMon sParty_Alexa[] = {
    {
    .iv = 100,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GLOOM,
    },
    {
    .iv = 100,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AZUMARILL,
    }
};

static const struct TrainerMon sParty_Ruben[] = {
    {
    .iv = 100,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHIFTRY,
    },
    {
    .iv = 100,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NOSEPASS,
    }
};

static const struct TrainerMon sParty_Koji1[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Wayne[] = {
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    }
};

static const struct TrainerMon sParty_Aidan[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    },
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKARMORY,
    }
};

static const struct TrainerMon sParty_Reed[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPHEAL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_Tisha[] = {
    {
    .nickname = _("Bidoofus"),
    .iv = 25,
    .lvl = 5,
    .evs = {20, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_BIDOOF,
    .moves = {MOVE_TACKLE, MOVE_DEFENSE_CURL, MOVE_WORK_UP, MOVE_RETURN},
    .nature = NATURE_IMPISH,
    .friendship = TRAINER_MON_FRIENDLY,
    .ball = ITEM_FRIEND_BALL,
    },
    {
    .nickname = _("Pawnidee"),
    .iv = 25,
    .lvl = 6,
    .evs = {0, 20, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_ROOKIDEE,
    .moves = {MOVE_SAND_ATTACK, MOVE_PECK, MOVE_TAILWIND, MOVE_ROOST},
    .nature = NATURE_BRAVE,
    .ball = ITEM_PREMIER_BALL,
    }
};

static const struct TrainerMon sParty_ToriAndTia[] = {
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPINDA,
    },
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPINDA,
    }
};

static const struct TrainerMon sParty_KimAndIris[] = {
    {
    .iv = 0,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWABLU,
    .moves = {MOVE_SING, MOVE_FURY_ATTACK, MOVE_SAFEGUARD, MOVE_AERIAL_ACE}
    },
    {
    .iv = 0,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    .moves = {MOVE_FLAMETHROWER, MOVE_TAKE_DOWN, MOVE_REST, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMon sParty_TyraAndIvy[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    .moves = {MOVE_GROWTH, MOVE_STUN_SPORE, MOVE_MEGA_DRAIN, MOVE_LEECH_SEED}
    },
    {
    .iv = 0,
    .lvl = 20,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GRAVELER,
    .moves = {MOVE_DEFENSE_CURL, MOVE_ROLLOUT, MOVE_MUD_SPORT, MOVE_ROCK_THROW}
    }
};

static const struct TrainerMon sParty_MelAndPaul[] = {
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSTOX,
    .moves = {MOVE_GUST, MOVE_PSYBEAM, MOVE_TOXIC, MOVE_PROTECT}
    },
    {
    .iv = 0,
    .lvl = 27,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BEAUTIFLY,
    .moves = {MOVE_GUST, MOVE_MEGA_DRAIN, MOVE_ATTRACT, MOVE_STUN_SPORE}
    }
};

static const struct TrainerMon sParty_JohnAndJay1[] = {
    {
    .iv = 200,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_PSYCHIC, MOVE_FIRE_PUNCH, MOVE_PSYCH_UP, MOVE_PROTECT}
    },
    {
    .iv = 200,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_FOCUS_PUNCH, MOVE_ROCK_TOMB, MOVE_REST, MOVE_BELLY_DRUM}
    }
};

static const struct TrainerMon sParty_JohnAndJay2[] = {
    {
    .iv = 210,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_PSYCHIC, MOVE_FIRE_PUNCH, MOVE_PSYCH_UP, MOVE_PROTECT}
    },
    {
    .iv = 210,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_FOCUS_PUNCH, MOVE_ROCK_TOMB, MOVE_REST, MOVE_BELLY_DRUM}
    }
};

static const struct TrainerMon sParty_JohnAndJay3[] = {
    {
    .iv = 220,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_PSYCHIC, MOVE_FIRE_PUNCH, MOVE_PSYCH_UP, MOVE_PROTECT}
    },
    {
    .iv = 220,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_FOCUS_PUNCH, MOVE_ROCK_TOMB, MOVE_REST, MOVE_BELLY_DRUM}
    }
};

static const struct TrainerMon sParty_JohnAndJay4[] = {
    {
    .iv = 230,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_PSYCHIC, MOVE_FIRE_PUNCH, MOVE_PSYCH_UP, MOVE_PROTECT}
    },
    {
    .iv = 230,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_FOCUS_PUNCH, MOVE_ROCK_TOMB, MOVE_REST, MOVE_BELLY_DRUM}
    }
};

static const struct TrainerMon sParty_JohnAndJay5[] = {
    {
    .iv = 240,
    .lvl = 52,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    .moves = {MOVE_PSYCHIC, MOVE_FIRE_PUNCH, MOVE_PSYCH_UP, MOVE_PROTECT}
    },
    {
    .iv = 240,
    .lvl = 52,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .moves = {MOVE_FOCUS_PUNCH, MOVE_ROCK_TOMB, MOVE_REST, MOVE_BELLY_DRUM}
    }
};

static const struct TrainerMon sParty_ReliAndIan[] = {
    {
    .iv = 0,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AZUMARILL,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMon sParty_LilaAndRoy1[] = {
    {
    .iv = 0,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CHINCHOU,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_LilaAndRoy2[] = {
    {
    .iv = 0,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CHINCHOU,
    },
    {
    .iv = 0,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_LilaAndRoy3[] = {
    {
    .iv = 0,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 0,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CARVANHA,
    }
};

static const struct TrainerMon sParty_LilaAndRoy4[] = {
    {
    .iv = 0,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 0,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_LilaAndRoy5[] = {
    {
    .iv = 0,
    .lvl = 51,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LANTURN,
    },
    {
    .iv = 0,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHARPEDO,
    }
};

static const struct TrainerMon sParty_LisaAndRay[] = {
    {
    .nickname = _("Nutmeg"),
    .iv = 200,
    .lvl = 26,
    .evs = {0, 252, 0, 4, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    .moves = {MOVE_SING, MOVE_ATTRACT, MOVE_RETURN, MOVE_PAYBACK},
    .gender = TRAINER_MON_FEMALE,
    .friendship = TRAINER_MON_FRIENDLY,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_LOVE_BALL,
    },
    {
    .nickname = _("Notmeg"),
    .iv = 200,
    .lvl = 28,
    .evs = {4, 252, 0, 0, 0, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_RATICATE_ALOLAN,
    .heldItem = ITEM_FLAME_ORB,
    .moves = {MOVE_HYPER_FANG, MOVE_SWITCHEROO, MOVE_WATER_GUN, MOVE_PLUCK},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_SPORT_BALL,
    }
};

static const struct TrainerMon sParty_Chris[] = {
    {
    .nickname = _("So Cute"),
    .iv = 200,
    .lvl = 26,
    .evs = {4, 252, 0, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DEERLING,
    .moves = {MOVE_LEECH_SEED, MOVE_JUMP_KICK, MOVE_BOUNCE, MOVE_PROTECT},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_HEAL_BALL,
    },
    {
    .nickname = _("Oh My Gosh"),
    .iv = 200,
    .lvl = 27,
    .evs = {4, 252, 0, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DEERLING_SUMMER,
    .moves = {MOVE_LEECH_SEED, MOVE_SOLAR_BEAM, MOVE_FEINT_ATTACK, MOVE_THUNDER_WAVE},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_HEAL_BALL,
    },
    {
    .nickname = _("No Way"),
    .iv = 200,
    .lvl = 28,
    .evs = {4, 252, 0, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DEERLING_AUTUMN,
    .moves = {MOVE_SOLAR_BEAM, MOVE_SWAGGER, MOVE_WILD_CHARGE, MOVE_SYNTHESIS},
    .nature = NATURE_MODEST,
    .ball = ITEM_FRIEND_BALL,
    },
    {
    .nickname = _("Adeerable"),
    .iv = 200,
    .lvl = 29,
    .evs = {4, 252, 0, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DEERLING_WINTER,
    .moves = {MOVE_SOLAR_BEAM, MOVE_TOXIC, MOVE_PROTECT, MOVE_LEECH_SEED},
    .nature = NATURE_MODEST,
    .ball = ITEM_FRIEND_BALL,
    }
};

static const struct TrainerMon sParty_Dawson[] = {
    {
    .iv = 0,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    .heldItem = ITEM_NUGGET
    },
    {
    .iv = 0,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    .heldItem = ITEM_NONE
    }
};

static const struct TrainerMon sParty_Sarah[] = {
    {
    .iv = 0,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOTAD,
    .heldItem = ITEM_NONE
    },
    {
    .iv = 0,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    .heldItem = ITEM_NUGGET
    }
};

static const struct TrainerMon sParty_Darian[] = {
    {
    .nickname = _("Blubby"),
    .iv = 100,
    .lvl = 20,
    .evs = {252, 0, 100, 0, 100, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    .moves = {MOVE_WATER_PULSE, MOVE_ICY_WIND, MOVE_BULLDOZE, MOVE_AQUA_RING},
    .nature = NATURE_MODEST,
    .ball = ITEM_DIVE_BALL,
    },
    {
    .nickname = _("Blobby"),
    .iv = 100,
    .lvl = 20,
    .evs = {252, 0, 100, 0, 100, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    .moves = {MOVE_AQUA_RING, MOVE_TOXIC, MOVE_ROCK_SMASH, MOVE_DIVE},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DIVE_BALL,
    }
};

static const struct TrainerMon sParty_Hailey[] = {
    {
    .iv = 0,
    .lvl = 13,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Chandler[] = {
    {
    .iv = 0,
    .lvl = 12,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    },
    {
    .iv = 0,
    .lvl = 12,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMon sParty_Kaleb[] = {
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MINUN,
    .heldItem = ITEM_ORAN_BERRY
    },
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PLUSLE,
    .heldItem = ITEM_ORAN_BERRY
    }
};

static const struct TrainerMon sParty_Joseph[] = {
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VOLTORB,
    }
};

static const struct TrainerMon sParty_Alyssa[] = {
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNEMITE,
    }
};

static const struct TrainerMon sParty_Marcos[] = {
    {
    .iv = 100,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VOLTORB,
    }
};

static const struct TrainerMon sParty_Rhett[] = {
    {
    .iv = 100,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Tyron[] = {
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    }
};

static const struct TrainerMon sParty_Celina[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Bianca[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    }
};

static const struct TrainerMon sParty_Hayden[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Sophie[] = {
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    }
};

static const struct TrainerMon sParty_Coby[] = {
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKARMORY,
    },
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMon sParty_Lawrence[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BALTOY,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    }
};

static const struct TrainerMon sParty_Wyatt[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARON,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARON,
    }
};

static const struct TrainerMon sParty_Angelina[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Kai[] = {
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BARBOACH,
    }
};

static const struct TrainerMon sParty_Charlotte[] = {
    {
    .iv = 0,
    .lvl = 19,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Deandre[] = {
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARON,
    },
    {
    .iv = 0,
    .lvl = 14,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRIKE,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout1[] = {
    {
    .nickname = _("Zorro"),
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZORUA,
    .moves = {MOVE_SCRATCH, MOVE_LEER, MOVE_TORMENT, MOVE_NONE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_SASSY,
    .ball = ITEM_DUSK_BALL,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout2[] = {
    {
    .nickname = _("Thiedore"),
    .iv = 0,
    .lvl = 23,
    .evs = {0, 0, 0, 60, 60, 252},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_THIEVUL,
    .moves = {MOVE_HONE_CLAWS, MOVE_PLAY_ROUGH, MOVE_ICE_FANG, MOVE_MUD_SHOT},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DUSK_BALL,
    },
    {
    .nickname = _("Melatonin"),
    .iv = 0,
    .lvl = 23,
    .evs = {0, 0, 0, 252, 60, 60},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_DROWZEE,
    .moves = {MOVE_POISON_GAS, MOVE_BRICK_BREAK, MOVE_DOUBLE_TEAM, MOVE_PSYSHOCK},
    .nature = NATURE_MODEST,
    .ball = ITEM_DREAM_BALL,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout3[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout4[] = {
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BALTOY,
    },
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout5[] = {
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BALTOY,
    },
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout6[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout7[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout8[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout9[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout10[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout11[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BALTOY,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout12[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout13[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout14[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout15[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_GruntMagmaHideout16[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BALTOY,
    }
};

static const struct TrainerMon sParty_TabithaMagmaHideout[] = {
    {
    .iv = 75,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 75,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 75,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZUBAT,
    },
    {
    .iv = 75,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMon sParty_Darcy[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMon sParty_MaxieMossdeep[] = {
    {
    .iv = 150,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 150,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CROBAT,
    },
    {
    .iv = 150,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    }
};

static const struct TrainerMon sParty_Pete[] = {
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMon sParty_Isabelle[] = {
    {
    .iv = 0,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    }
};

static const struct TrainerMon sParty_Andres1[] = {
    {
    .iv = 50,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 50,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    }
};

static const struct TrainerMon sParty_Josue[] = {
    {
    .iv = 50,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = 50,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    }
};

static const struct TrainerMon sParty_Camron[] = {
    {
    .nickname = _("Guinevere"),
    .iv = 100,
    .lvl = 29,
    .evs = {4, 0, 0, 252, 0, 252},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_INDEEDEE_FEMALE,
    .moves = {MOVE_PSYBEAM, MOVE_MAGICAL_LEAF, MOVE_DRAINING_KISS, MOVE_MYSTICAL_FIRE},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_DREAM_BALL,
    },
    {
    .nickname = _("Morgana"),
    .iv = 100,
    .lvl = 29,
    .evs = {4, 0, 0, 252, 0, 252},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_MEOWSTIC_FEMALE,
    .moves = {MOVE_TRICK, MOVE_PSYBEAM, MOVE_PSYCHIC_TERRAIN, MOVE_SHADOW_BALL},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_DREAM_BALL,
    },
};

static const struct TrainerMon sParty_Cory1[] = {
    {
    .iv = 200,
    .lvl = 30,
    .evs = {0, 252, 0, 252, 0, 4},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_GOLDUCK,
    },
    {
    .iv = 200,
    .lvl = 30,
    .evs = {0, 252, 0, 252, 0, 4},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_PSYDUCK,
    }
};

static const struct TrainerMon sParty_Carolina[] = {
    {
    .nickname = _("Nightfang"),
    .iv = 200,
    .lvl = 28,
    .evs = {0, 252, 0, 0, 4, 252},
    .ability = ABILITY_SLOT_1,
    .heldItem = ITEM_LIFE_ORB,
    .species = SPECIES_MIGHTYENA,
    .moves = {MOVE_CRUNCH, MOVE_SUCKER_PUNCH, MOVE_PLAY_ROUGH, MOVE_IRON_TAIL},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname = _("Toxin"),
    .iv = 200,
    .lvl = 28,
    .evs = {252, 0, 4, 252, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_GRIMER,
    .moves = {MOVE_THIEF, MOVE_SLUDGE_WAVE, MOVE_TOXIC, MOVE_FLAMETHROWER},
    .nature = NATURE_JOLLY,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname = _("Slice"),
    .iv = 200,
    .lvl = 28,
    .evs = {4, 252, 0, 0, 0, 252},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_GLISCOR,
    .heldItem = ITEM_TOXIC_ORB,
    .moves = {MOVE_SWORDS_DANCE, MOVE_ROOST, MOVE_EARTHQUAKE, MOVE_FACADE},
    .nature = NATURE_JOLLY,
    .ball = ITEM_GREAT_BALL,
    }
};

static const struct TrainerMon sParty_Elijah[] = {
    {
    .nickname = _("Pokymon"),
    .iv = 255,
    .lvl = 33,
    .evs = {252, 0, 252, 4, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_PINCURCHIN,
    .moves = {MOVE_RISING_VOLTAGE, MOVE_SCALD, MOVE_RECOVER, MOVE_SPIKES},
    .nature = NATURE_BOLD,
    .ball = ITEM_FAST_BALL,
    },
    {
    .nickname = _("Morpopo"),
    .iv = 255,
    .lvl = 33,
    .evs = {0, 252, 4, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MORPEKO,
    .moves = {MOVE_AURA_WHEEL, MOVE_CRUNCH, MOVE_SUPER_FANG, MOVE_PROTECT},
    .nature = NATURE_JOLLY,
    .ball = ITEM_FAST_BALL,
    },
    {
    .nickname = _("Dinoshock"),
    .iv = 255,
    .lvl = 33,
    .evs = {0, 182, 0, 140, 0, 184},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DRACOZOLT,
    .moves = {MOVE_BOLT_BEAK, MOVE_DRAGON_RAGE, MOVE_SUBSTITUTE, MOVE_FIRE_SPIN},
    .nature = NATURE_LONELY,
    .ball = ITEM_REPEAT_BALL,
    }
};

static const struct TrainerMon sParty_Celia[] = {
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    }
};

static const struct TrainerMon sParty_Bryan[] = {
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSLASH,
    }
};

static const struct TrainerMon sParty_Branden[] = {
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TAILLOW,
    },
    {
    .iv = 0,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUZLEAF,
    }
};

static const struct TrainerMon sParty_Bryant[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    }
};

static const struct TrainerMon sParty_Shayla[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    }
};

static const struct TrainerMon sParty_Kyra[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODUO,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODRIO,
    }
};

static const struct TrainerMon sParty_Jaiden[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NINJASK,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GULPIN,
    }
};

static const struct TrainerMon sParty_Alix[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KIRLIA,
    }
};

static const struct TrainerMon sParty_Helene[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Marlene[] = {
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 0,
    .lvl = 18,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPOINK,
    }
};

static const struct TrainerMon sParty_Devan[] = {
    {
    .iv = 0,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    },
    {
    .iv = 0,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GEODUDE,
    }
};

static const struct TrainerMon sParty_Johnson[] = {
    {
    .iv = 0,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 0,
    .lvl = 8,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOTAD,
    }
};

static const struct TrainerMon sParty_Melina[] = {
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DODUO,
    }
};

static const struct TrainerMon sParty_Brandi[] = {
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RALTS,
    }
};

static const struct TrainerMon sParty_Aisha[] = {
    {
    .iv = 0,
    .lvl = 17,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    }
};

static const struct TrainerMon sParty_Makayla[] = {
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ROSELIA,
    },
    {
    .iv = 0,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    }
};

static const struct TrainerMon sParty_Fabian[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    }
};

static const struct TrainerMon sParty_Dayton[] = {
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLUGMA,
    },
    {
    .iv = 0,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Rachel[] = {
    {
    .iv = 0,
    .lvl = 26,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLDEEN,
    }
};

static const struct TrainerMon sParty_Leonel[] = {
    {
    .iv = 100,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    .moves = {MOVE_THUNDER, MOVE_QUICK_ATTACK, MOVE_THUNDER_WAVE, MOVE_NONE}
    }
};

static const struct TrainerMon sParty_Callie[] = {
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    },
    {
    .iv = 0,
    .lvl = 28,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    }
};

static const struct TrainerMon sParty_Cale[] = {
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DUSTOX,
    },
    {
    .iv = 0,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BEAUTIFLY,
    }
};

static const struct TrainerMon sParty_Myles[] = {
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUMEL,
    }
};

static const struct TrainerMon sParty_Pat[] = {
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POOCHYENA,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHROOMISH,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MARILL,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 10,
    .lvl = 25,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GULPIN,
    }
};

static const struct TrainerMon sParty_Cristin1[] = {
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 100,
    .lvl = 29,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VIGOROTH,
    }
};

static const struct TrainerMon sParty_MayRustboroTreecko[] = {
{
    .nickname = _("Allister"),
    .iv = 255,
    .lvl = 15,
    .evs = {4, 0, 0, 252, 0, 252},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_LITWICK,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HEX, MOVE_WILL_O_WISP, MOVE_CONFUSE_RAY, MOVE_SUBSTITUTE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_GREAT_BALL,
    },
    {
    .nickname =_("Nessa"),
    .iv = 255,
    .lvl = 16,
    .evs = {52, 252, 0, 0, 0, 204},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CHINCHOU,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_BUBBLE_BEAM, MOVE_THUNDER_WAVE, MOVE_ELECTRO_BALL, MOVE_SUPERSONIC},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_MODEST,
    .ball = ITEM_DIVE_BALL,
    },
    {
    .nickname = _("Leon"),
    .iv = 255,
    .lvl = 15,
    .evs = {0, 252, 4, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BEEDRILL,
    .heldItem = ITEM_BEEDRILLITE,
    .moves = {MOVE_DRILL_RUN, MOVE_FURY_ATTACK, MOVE_POISON_STING, MOVE_PROTECT},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_JOLLY,
    .ball = ITEM_CHERISH_BALL,
    }
};

static const struct TrainerMon sParty_MayRustboroTorchic[] = {
    {
    .iv = 25,
    .lvl = 13,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TORKOAL,
    },
    {
    .iv = 50,
    .lvl = 15,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MUDKIP,
    }
};

static const struct TrainerMon sParty_Roxanne2[] = {
    {
    .iv = 255,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLEM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PROTECT, MOVE_ROLLOUT, MOVE_MAGNITUDE, MOVE_EXPLOSION}
    },
    {
    .iv = 255,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KABUTO,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_ICE_BEAM, MOVE_SURF, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ONIX,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_IRON_TAIL, MOVE_EXPLOSION, MOVE_ROAR, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NOSEPASS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_EXPLOSION, MOVE_PROTECT, MOVE_ROCK_SLIDE}
    }
};

static const struct TrainerMon sParty_Roxanne3[] = {
    {
    .iv = 255,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_OMANYTE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PROTECT, MOVE_ICE_BEAM, MOVE_ROCK_SLIDE, MOVE_SURF}
    },
    {
    .iv = 255,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLEM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PROTECT, MOVE_ROLLOUT, MOVE_MAGNITUDE, MOVE_EXPLOSION}
    },
    {
    .iv = 255,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KABUTOPS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_ICE_BEAM, MOVE_SURF, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ONIX,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_IRON_TAIL, MOVE_EXPLOSION, MOVE_ROAR, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NOSEPASS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_EXPLOSION, MOVE_PROTECT, MOVE_ROCK_SLIDE}
    }
};

static const struct TrainerMon sParty_Roxanne4[] = {
    {
    .iv = 255,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_OMASTAR,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PROTECT, MOVE_ICE_BEAM, MOVE_ROCK_SLIDE, MOVE_SURF}
    },
    {
    .iv = 255,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLEM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PROTECT, MOVE_ROLLOUT, MOVE_EARTHQUAKE, MOVE_EXPLOSION}
    },
    {
    .iv = 255,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KABUTOPS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_ICE_BEAM, MOVE_SURF, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ONIX,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_IRON_TAIL, MOVE_EXPLOSION, MOVE_ROAR, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 47,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NOSEPASS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_EXPLOSION, MOVE_PROTECT, MOVE_ROCK_SLIDE}
    }
};

static const struct TrainerMon sParty_Roxanne5[] = {
    {
    .iv = 255,
    .lvl = 47,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AERODACTYL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ROCK_SLIDE, MOVE_HYPER_BEAM, MOVE_SUPERSONIC, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 47,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GOLEM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FOCUS_PUNCH, MOVE_ROLLOUT, MOVE_EARTHQUAKE, MOVE_EXPLOSION}
    },
    {
    .iv = 255,
    .lvl = 47,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_OMASTAR,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PROTECT, MOVE_ICE_BEAM, MOVE_ROCK_SLIDE, MOVE_SURF}
    },
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KABUTOPS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SWORDS_DANCE, MOVE_ICE_BEAM, MOVE_SURF, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STEELIX,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_IRON_TAIL, MOVE_EXPLOSION, MOVE_ROAR, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 52,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NOSEPASS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_DOUBLE_TEAM, MOVE_EXPLOSION, MOVE_PROTECT, MOVE_ROCK_SLIDE}
    }
};

static const struct TrainerMon sParty_Brawly2[] = {
    {
    .iv = 255,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_KARATE_CHOP, MOVE_ROCK_SLIDE, MOVE_FOCUS_PUNCH, MOVE_BULK_UP}
    },
    {
    .iv = 255,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDITITE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PSYCHIC, MOVE_LIGHT_SCREEN, MOVE_REFLECT, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HITMONTOP,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PURSUIT, MOVE_COUNTER, MOVE_PROTECT, MOVE_TRIPLE_KICK}
    },
    {
    .iv = 255,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_FAKE_OUT, MOVE_FOCUS_PUNCH, MOVE_BELLY_DRUM, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMon sParty_Brawly3[] = {
    {
    .iv = 255,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_KARATE_CHOP, MOVE_ROCK_SLIDE, MOVE_FOCUS_PUNCH, MOVE_BULK_UP}
    },
    {
    .iv = 255,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PSYCHIC, MOVE_LIGHT_SCREEN, MOVE_REFLECT, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HITMONTOP,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PURSUIT, MOVE_COUNTER, MOVE_PROTECT, MOVE_TRIPLE_KICK}
    },
    {
    .iv = 255,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_FAKE_OUT, MOVE_FOCUS_PUNCH, MOVE_BELLY_DRUM, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMon sParty_Brawly4[] = {
    {
    .iv = 255,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HITMONCHAN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SKY_UPPERCUT, MOVE_PROTECT, MOVE_FIRE_PUNCH, MOVE_ICE_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_KARATE_CHOP, MOVE_ROCK_SLIDE, MOVE_FOCUS_PUNCH, MOVE_BULK_UP}
    },
    {
    .iv = 255,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FOCUS_PUNCH, MOVE_LIGHT_SCREEN, MOVE_REFLECT, MOVE_PSYCHIC}
    },
    {
    .iv = 255,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HITMONTOP,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PURSUIT, MOVE_COUNTER, MOVE_PROTECT, MOVE_TRIPLE_KICK}
    },
    {
    .iv = 255,
    .lvl = 47,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_FAKE_OUT, MOVE_FOCUS_PUNCH, MOVE_BELLY_DRUM, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMon sParty_Brawly5[] = {
    {
    .iv = 255,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HITMONLEE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_MEGA_KICK, MOVE_FOCUS_PUNCH, MOVE_EARTHQUAKE, MOVE_BULK_UP}
    },
    {
    .iv = 255,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HITMONCHAN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SKY_UPPERCUT, MOVE_PROTECT, MOVE_FIRE_PUNCH, MOVE_ICE_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHAMP,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_CROSS_CHOP, MOVE_ROCK_SLIDE, MOVE_FOCUS_PUNCH, MOVE_BULK_UP}
    },
    {
    .iv = 255,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MEDICHAM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FOCUS_PUNCH, MOVE_LIGHT_SCREEN, MOVE_REFLECT, MOVE_PSYCHIC}
    },
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HITMONTOP,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PURSUIT, MOVE_COUNTER, MOVE_PROTECT, MOVE_TRIPLE_KICK}
    },
    {
    .iv = 255,
    .lvl = 52,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_FAKE_OUT, MOVE_FOCUS_PUNCH, MOVE_BELLY_DRUM, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMon sParty_Wattson2[] = {
    {
    .iv = 255,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAREEP,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_THUNDER, MOVE_PROTECT, MOVE_THUNDER_WAVE, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRODE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ROLLOUT, MOVE_THUNDER, MOVE_EXPLOSION, MOVE_RAIN_DANCE}
    },
    {
    .iv = 255,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_THUNDER, MOVE_RAIN_DANCE}
    },
    {
    .iv = 255,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BITE, MOVE_THUNDER_WAVE, MOVE_THUNDER, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Wattson3[] = {
    {
    .iv = 255,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PIKACHU,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_THUNDER, MOVE_SLAM, MOVE_RAIN_DANCE, MOVE_SHOCK_WAVE}
    },
    {
    .iv = 255,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_FLAAFFY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_THUNDER, MOVE_PROTECT, MOVE_THUNDER_WAVE, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRODE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ROLLOUT, MOVE_THUNDER, MOVE_EXPLOSION, MOVE_RAIN_DANCE}
    },
    {
    .iv = 255,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_THUNDER, MOVE_RAIN_DANCE}
    },
    {
    .iv = 255,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BITE, MOVE_THUNDER_WAVE, MOVE_THUNDER, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Wattson4[] = {
    {
    .iv = 255,
    .lvl = 44,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RAICHU,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_THUNDER, MOVE_SLAM, MOVE_RAIN_DANCE, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_THUNDER, MOVE_PROTECT, MOVE_THUNDER_WAVE, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRODE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ROLLOUT, MOVE_THUNDER, MOVE_EXPLOSION, MOVE_RAIN_DANCE}
    },
    {
    .iv = 255,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_THUNDER, MOVE_RAIN_DANCE}
    },
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BITE, MOVE_THUNDER_WAVE, MOVE_THUNDER, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Wattson5[] = {
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTABUZZ,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SWIFT, MOVE_FOCUS_PUNCH, MOVE_THUNDER_PUNCH, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 51,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RAICHU,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_THUNDER, MOVE_SLAM, MOVE_RAIN_DANCE, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 51,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AMPHAROS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_THUNDER, MOVE_PROTECT, MOVE_THUNDER_WAVE, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRODE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ROLLOUT, MOVE_THUNDER, MOVE_EXPLOSION, MOVE_RAIN_DANCE}
    },
    {
    .iv = 255,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGNETON,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_THUNDER, MOVE_RAIN_DANCE}
    },
    {
    .iv = 255,
    .lvl = 55,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BITE, MOVE_THUNDER_WAVE, MOVE_THUNDER, MOVE_PROTECT}
    }
};

static const struct TrainerMon sParty_Flannery2[] = {
    {
    .iv = 255,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGCARGO,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_ATTRACT, MOVE_LIGHT_SCREEN, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PONYTA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_SOLAR_BEAM, MOVE_BOUNCE}
    },
    {
    .iv = 255,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EARTHQUAKE, MOVE_ATTRACT}
    },
    {
    .iv = 255,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EXPLOSION, MOVE_ATTRACT}
    }
};

static const struct TrainerMon sParty_Flannery3[] = {
    {
    .iv = 255,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GROWLITHE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HELPING_HAND, MOVE_FLAMETHROWER, MOVE_ROAR, MOVE_SUNNY_DAY}
    },
    {
    .iv = 255,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGCARGO,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_ATTRACT, MOVE_LIGHT_SCREEN, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PONYTA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_SOLAR_BEAM, MOVE_BOUNCE}
    },
    {
    .iv = 255,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EARTHQUAKE, MOVE_ATTRACT}
    },
    {
    .iv = 255,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EXPLOSION, MOVE_ATTRACT}
    }
};

static const struct TrainerMon sParty_Flannery4[] = {
    {
    .iv = 255,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HOUNDOUR,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ROAR, MOVE_SOLAR_BEAM, MOVE_TAUNT, MOVE_SUNNY_DAY}
    },
    {
    .iv = 255,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GROWLITHE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HELPING_HAND, MOVE_FLAMETHROWER, MOVE_SUNNY_DAY, MOVE_ROAR}
    },
    {
    .iv = 255,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGCARGO,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_ATTRACT, MOVE_LIGHT_SCREEN, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RAPIDASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_SOLAR_BEAM, MOVE_BOUNCE}
    },
    {
    .iv = 255,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EARTHQUAKE, MOVE_ATTRACT}
    },
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EXPLOSION, MOVE_ATTRACT}
    }
};

static const struct TrainerMon sParty_Flannery5[] = {
    {
    .iv = 255,
    .lvl = 51,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARCANINE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HELPING_HAND, MOVE_FLAMETHROWER, MOVE_SUNNY_DAY, MOVE_ROAR}
    },
    {
    .iv = 255,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAGCARGO,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_ATTRACT, MOVE_LIGHT_SCREEN, MOVE_ROCK_SLIDE}
    },
    {
    .iv = 255,
    .lvl = 51,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HOUNDOOM,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_ROAR, MOVE_SOLAR_BEAM, MOVE_TAUNT, MOVE_SUNNY_DAY}
    },
    {
    .iv = 255,
    .lvl = 51,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_RAPIDASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FLAMETHROWER, MOVE_ATTRACT, MOVE_SOLAR_BEAM, MOVE_BOUNCE}
    },
    {
    .iv = 255,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CAMERUPT,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EARTHQUAKE, MOVE_ATTRACT}
    },
    {
    .iv = 255,
    .lvl = 55,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TORKOAL,
    .heldItem = ITEM_WHITE_HERB,
    .moves = {MOVE_OVERHEAT, MOVE_SUNNY_DAY, MOVE_EXPLOSION, MOVE_ATTRACT}
    }
};

static const struct TrainerMon sParty_Norman2[] = {
    {
    .iv = 255,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CHANSEY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_SING, MOVE_SKILL_SWAP, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 42,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_DOUBLE_EDGE, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TEETER_DANCE, MOVE_SKILL_SWAP, MOVE_FACADE, MOVE_HYPNOSIS}
    },
    {
    .iv = 255,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HYPER_BEAM, MOVE_FLAMETHROWER, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMon sParty_Norman3[] = {
    {
    .iv = 255,
    .lvl = 47,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_DOUBLE_EDGE, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 47,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CHANSEY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_SING, MOVE_SKILL_SWAP, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FAKE_OUT, MOVE_DIZZY_PUNCH, MOVE_ENDURE, MOVE_REVERSAL}
    },
    {
    .iv = 255,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TEETER_DANCE, MOVE_SKILL_SWAP, MOVE_FACADE, MOVE_HYPNOSIS}
    },
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HYPER_BEAM, MOVE_FLAMETHROWER, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMon sParty_Norman4[] = {
    {
    .iv = 255,
    .lvl = 52,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_DOUBLE_EDGE, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 52,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BLISSEY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_LIGHT_SCREEN, MOVE_SING, MOVE_SKILL_SWAP, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FAKE_OUT, MOVE_DIZZY_PUNCH, MOVE_ENDURE, MOVE_REVERSAL}
    },
    {
    .iv = 255,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TEETER_DANCE, MOVE_SKILL_SWAP, MOVE_FACADE, MOVE_HYPNOSIS}
    },
    {
    .iv = 255,
    .lvl = 55,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HYPER_BEAM, MOVE_FLAMETHROWER, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMon sParty_Norman5[] = {
    {
    .iv = 255,
    .lvl = 57,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_BLIZZARD, MOVE_SHADOW_BALL, MOVE_DOUBLE_EDGE, MOVE_FIRE_BLAST}
    },
    {
    .iv = 255,
    .lvl = 57,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BLISSEY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_PROTECT, MOVE_SING, MOVE_SKILL_SWAP, MOVE_FOCUS_PUNCH}
    },
    {
    .iv = 255,
    .lvl = 55,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KANGASKHAN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_FAKE_OUT, MOVE_DIZZY_PUNCH, MOVE_ENDURE, MOVE_REVERSAL}
    },
    {
    .iv = 255,
    .lvl = 57,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TAUROS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TAKE_DOWN, MOVE_PROTECT, MOVE_FIRE_BLAST, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 58,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPINDA,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TEETER_DANCE, MOVE_SKILL_SWAP, MOVE_FACADE, MOVE_HYPNOSIS}
    },
    {
    .iv = 255,
    .lvl = 60,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HYPER_BEAM, MOVE_FLAMETHROWER, MOVE_THUNDERBOLT, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMon sParty_Winona2[] = {
    {
    .iv = 255,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DRATINI,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDER_WAVE, MOVE_THUNDERBOLT, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SUNNY_DAY, MOVE_AERIAL_ACE, MOVE_SOLAR_BEAM, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_AERIAL_ACE, MOVE_REST, MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMon sParty_Winona3[] = {
    {
    .iv = 255,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HOOTHOOT,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_PSYCHIC, MOVE_REFLECT, MOVE_DREAM_EATER}
    },
    {
    .iv = 255,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SUNNY_DAY, MOVE_AERIAL_ACE, MOVE_SOLAR_BEAM, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 45,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DRAGONAIR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDER_WAVE, MOVE_THUNDERBOLT, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_AERIAL_ACE, MOVE_REST, MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMon sParty_Winona4[] = {
    {
    .iv = 255,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NOCTOWL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_PSYCHIC, MOVE_REFLECT, MOVE_DREAM_EATER}
    },
    {
    .iv = 255,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SUNNY_DAY, MOVE_AERIAL_ACE, MOVE_SOLAR_BEAM, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DRAGONAIR,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_THUNDER_WAVE, MOVE_THUNDERBOLT, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 51,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 55,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_AERIAL_ACE, MOVE_REST, MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMon sParty_Winona5[] = {
    {
    .iv = 255,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NOCTOWL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_PSYCHIC, MOVE_REFLECT, MOVE_DREAM_EATER}
    },
    {
    .iv = 255,
    .lvl = 54,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SUNNY_DAY, MOVE_AERIAL_ACE, MOVE_SOLAR_BEAM, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 55,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_SURF, MOVE_SUPERSONIC, MOVE_PROTECT, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 55,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DRAGONITE,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_HYPER_BEAM, MOVE_THUNDERBOLT, MOVE_EARTHQUAKE, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 58,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WHIRLWIND, MOVE_SPIKES, MOVE_STEEL_WING, MOVE_AERIAL_ACE}
    },
    {
    .iv = 255,
    .lvl = 60,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ALTARIA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_SKY_ATTACK, MOVE_REST, MOVE_DRAGON_DANCE, MOVE_EARTHQUAKE}
    }
};

static const struct TrainerMon sParty_TateAndLiza2[] = {
    {
    .iv = 255,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLOWPOKE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_EARTHQUAKE, MOVE_ANCIENT_POWER, MOVE_PSYCHIC, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 49,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_XATU,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_REST, MOVE_CONFUSE_RAY, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_REST, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM, MOVE_PSYCHIC, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMon sParty_TateAndLiza3[] = {
    {
    .iv = 255,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DROWZEE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_DREAM_EATER, MOVE_HEADBUTT, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLOWPOKE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 54,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_EARTHQUAKE, MOVE_EXPLOSION, MOVE_PSYCHIC, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 54,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_XATU,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_REST, MOVE_CONFUSE_RAY, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 55,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_REST, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 55,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM, MOVE_PSYCHIC, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMon sParty_TateAndLiza4[] = {
    {
    .iv = 255,
    .lvl = 58,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HYPNO,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_DREAM_EATER, MOVE_HEADBUTT, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 59,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_EARTHQUAKE, MOVE_EXPLOSION, MOVE_PSYCHIC, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 58,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLOWPOKE,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 59,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_XATU,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_REST, MOVE_CONFUSE_RAY, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 60,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_REST, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 60,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM, MOVE_PSYCHIC, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMon sParty_TateAndLiza5[] = {
    {
    .iv = 255,
    .lvl = 63,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HYPNO,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_DREAM_EATER, MOVE_HEADBUTT, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 64,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_EARTHQUAKE, MOVE_EXPLOSION, MOVE_PSYCHIC, MOVE_LIGHT_SCREEN}
    },
    {
    .iv = 255,
    .lvl = 63,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLOWKING,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_YAWN, MOVE_PSYCHIC, MOVE_CALM_MIND, MOVE_PROTECT}
    },
    {
    .iv = 255,
    .lvl = 64,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_XATU,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_PSYCHIC, MOVE_REST, MOVE_CONFUSE_RAY, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 65,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUNATONE,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_REST, MOVE_CALM_MIND}
    },
    {
    .iv = 255,
    .lvl = 65,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SOLROCK,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_SUNNY_DAY, MOVE_SOLAR_BEAM, MOVE_PSYCHIC, MOVE_FLAMETHROWER}
    }
};

static const struct TrainerMon sParty_Juan2[] = {
    {
    .iv = 255,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POLIWAG,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_RAIN_DANCE, MOVE_PROTECT, MOVE_HYDRO_PUMP}
    },
    {
    .iv = 255,
    .lvl = 46,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_FISSURE}
    },
    {
    .iv = 255,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATER_PULSE, MOVE_BODY_SLAM, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 48,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_REST, MOVE_CRABHAMMER, MOVE_TAUNT, MOVE_DOUBLE_TEAM}
    },
    {
    .iv = 255,
    .lvl = 51,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMon sParty_Juan3[] = {
    {
    .iv = 255,
    .lvl = 50,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POLIWHIRL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_RAIN_DANCE, MOVE_PROTECT, MOVE_HYDRO_PUMP}
    },
    {
    .iv = 255,
    .lvl = 51,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_FISSURE}
    },
    {
    .iv = 255,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATER_PULSE, MOVE_BODY_SLAM, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 53,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_REST, MOVE_GUILLOTINE, MOVE_TAUNT, MOVE_DOUBLE_TEAM}
    },
    {
    .iv = 255,
    .lvl = 56,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMon sParty_Juan4[] = {
    {
    .iv = 255,
    .lvl = 56,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LAPRAS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYDRO_PUMP, MOVE_PERISH_SONG, MOVE_ICE_BEAM, MOVE_CONFUSE_RAY}
    },
    {
    .iv = 255,
    .lvl = 58,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_FISSURE}
    },
    {
    .iv = 255,
    .lvl = 56,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POLIWHIRL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_RAIN_DANCE, MOVE_PROTECT, MOVE_HYDRO_PUMP}
    },
    {
    .iv = 255,
    .lvl = 58,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATER_PULSE, MOVE_BODY_SLAM, MOVE_PROTECT, MOVE_ICE_BEAM}
    },
    {
    .iv = 255,
    .lvl = 58,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_REST, MOVE_GUILLOTINE, MOVE_TAUNT, MOVE_DOUBLE_TEAM}
    },
    {
    .iv = 255,
    .lvl = 61,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMon sParty_Juan5[] = {
    {
    .iv = 255,
    .lvl = 61,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LAPRAS,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYDRO_PUMP, MOVE_PERISH_SONG, MOVE_ICE_BEAM, MOVE_CONFUSE_RAY}
    },
    {
    .iv = 255,
    .lvl = 63,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WHISCASH,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_RAIN_DANCE, MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_FISSURE}
    },
    {
    .iv = 255,
    .lvl = 61,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_POLITOED,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_HYPNOSIS, MOVE_RAIN_DANCE, MOVE_HYDRO_PUMP, MOVE_PERISH_SONG}
    },
    {
    .iv = 255,
    .lvl = 63,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WALREIN,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATER_PULSE, MOVE_BODY_SLAM, MOVE_PROTECT, MOVE_SHEER_COLD}
    },
    {
    .iv = 255,
    .lvl = 63,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CRAWDAUNT,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_REST, MOVE_GUILLOTINE, MOVE_TAUNT, MOVE_DOUBLE_TEAM}
    },
    {
    .iv = 255,
    .lvl = 66,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KINGDRA,
    .heldItem = ITEM_CHESTO_BERRY,
    .moves = {MOVE_WATER_PULSE, MOVE_DOUBLE_TEAM, MOVE_ICE_BEAM, MOVE_REST}
    }
};

static const struct TrainerMon sParty_Angelo[] = {
    {
    .nickname = _("Helvetica"),
    .iv = 100,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CUFANT,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_ROLLOUT, MOVE_MUD_SHOT, MOVE_REST, MOVE_ROCK_SMASH},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_HEAVY_BALL,
    },
    {
    .nickname = _("Barb"),
    .iv = 100,
    .lvl = 22,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_FERROSEED,
    .heldItem = ITEM_LEFTOVERS,
    .moves = {MOVE_INGRAIN, MOVE_METAL_CLAW, MOVE_THUNDER_WAVE, MOVE_BULLET_SEED},
    .gender = TRAINER_MON_FEMALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_HEAVY_BALL,
    }
};

static const struct TrainerMon sParty_Darius[] = {
    {
    .iv = 200,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TROPIUS,
    }
};

static const struct TrainerMon sParty_Steven[] = {
    {
    .iv = 255,
    .lvl = 77,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKARMORY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_TOXIC, MOVE_AERIAL_ACE, MOVE_SPIKES, MOVE_STEEL_WING}
    },
    {
    .iv = 255,
    .lvl = 75,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CLAYDOL,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_REFLECT, MOVE_LIGHT_SCREEN, MOVE_ANCIENT_POWER, MOVE_EARTHQUAKE}
    },
    {
    .iv = 255,
    .lvl = 76,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_AGGRON,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_THUNDER, MOVE_EARTHQUAKE, MOVE_SOLAR_BEAM, MOVE_DRAGON_CLAW}
    },
    {
    .iv = 255,
    .lvl = 76,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CRADILY,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_GIGA_DRAIN, MOVE_ANCIENT_POWER, MOVE_INGRAIN, MOVE_CONFUSE_RAY}
    },
    {
    .iv = 255,
    .lvl = 76,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ARMALDO,
    .heldItem = ITEM_NONE,
    .moves = {MOVE_WATER_PULSE, MOVE_ANCIENT_POWER, MOVE_AERIAL_ACE, MOVE_SLASH}
    },
    {
    .iv = 255,
    .lvl = 78,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_METAGROSS,
    .heldItem = ITEM_SITRUS_BERRY,
    .moves = {MOVE_EARTHQUAKE, MOVE_PSYCHIC, MOVE_METEOR_MASH, MOVE_SHADOW_BALL}
    }
};

static const struct TrainerMon sParty_Anabel[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Tucker[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Spenser[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Greta[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Noland[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Lucy[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Brandon[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BELDUM,
    }
};

static const struct TrainerMon sParty_Andres2[] = {
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    }
};

static const struct TrainerMon sParty_Andres3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NOSEPASS,
    },
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    }
};

static const struct TrainerMon sParty_Andres4[] = {
    {
    .iv = 30,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NOSEPASS,
    },
    {
    .iv = 30,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    },
    {
    .iv = 30,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSHREW,
    }
};

static const struct TrainerMon sParty_Andres5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NOSEPASS,
    },
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSLASH,
    },
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SANDSLASH,
    }
};

static const struct TrainerMon sParty_Cory2[] = {
    {
    .nickname = _("Gullible"),
    .iv = 100,
    .lvl = 30,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    .moves = {MOVE_SCALD, MOVE_U_TURN, MOVE_ROOST, MOVE_DEFOG},
    .nature = NATURE_BOLD,
    .ball = ITEM_DIVE_BALL,
    },
    {
    .nickname = _("Primeapple"),
    .iv = 100,
    .lvl = 30,
    .evs = {0, 252, 4, 0, 0, 252},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PRIMEAPE,
    .moves = {MOVE_CLOSE_COMBAT, MOVE_U_TURN, MOVE_STONE_EDGE, MOVE_GUNK_SHOT},
    .nature = NATURE_JOLLY,
    .ball = ITEM_ULTRA_BALL,
    },
    {
    .nickname = _("Tentapole"),
    .iv = 100,
    .lvl = 30,
    .evs = {252, 0, 112, 0, 0, 114},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    .heldItem = ITEM_BLACK_SLUDGE,
    .moves = {MOVE_SCALD, MOVE_RAPID_SPIN, MOVE_HAZE, MOVE_SLUDGE_BOMB},
    .nature = NATURE_TIMID,
    .ball = ITEM_DIVE_BALL,
    }
};

static const struct TrainerMon sParty_Cory3[] = {
    {
    .iv = 20,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 20,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 20,
    .lvl = 32,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACOOL,
    }
};

static const struct TrainerMon sParty_Cory4[] = {
    {
    .iv = 30,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 30,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOP,
    },
    {
    .iv = 30,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Cory5[] = {
    {
    .iv = 40,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 40,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 40,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TENTACRUEL,
    }
};

static const struct TrainerMon sParty_Pablo2[] = {
    {
    .iv = 10,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    },
    {
    .iv = 10,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Pablo3[] = {
    {
    .iv = 20,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WINGULL,
    },
    {
    .iv = 20,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    },
    {
    .iv = 20,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Pablo4[] = {
    {
    .iv = 30,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 30,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    },
    {
    .iv = 30,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARYU,
    }
};

static const struct TrainerMon sParty_Pablo5[] = {
    {
    .iv = 40,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_PELIPPER,
    },
    {
    .iv = 40,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARMIE,
    },
    {
    .iv = 40,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_STARMIE,
    }
};

static const struct TrainerMon sParty_Koji2[] = {
    {
    .iv = 10,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 10,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Koji3[] = {
    {
    .iv = 20,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MAKUHITA,
    },
    {
    .iv = 20,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 20,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Koji4[] = {
    {
    .iv = 30,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    },
    {
    .iv = 30,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    },
    {
    .iv = 30,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHOKE,
    }
};

static const struct TrainerMon sParty_Koji5[] = {
    {
    .iv = 40,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HARIYAMA,
    },
    {
    .iv = 40,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHAMP,
    },
    {
    .iv = 40,
    .lvl = 43,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MACHAMP,
    }
};

static const struct TrainerMon sParty_Cristin2[] = {
    {
    .iv = 110,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 110,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VIGOROTH,
    }
};

static const struct TrainerMon sParty_Cristin3[] = {
    {
    .iv = 120,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPINDA,
    },
    {
    .iv = 120,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 120,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VIGOROTH,
    }
};

static const struct TrainerMon sParty_Cristin4[] = {
    {
    .iv = 130,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPINDA,
    },
    {
    .iv = 130,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    },
    {
    .iv = 100,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_VIGOROTH,
    }
};

static const struct TrainerMon sParty_Cristin5[] = {
    {
    .iv = 140,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SPINDA,
    },
    {
    .iv = 140,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_EXPLOUD,
    },
    {
    .iv = 140,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SLAKING,
    }
};

static const struct TrainerMon sParty_Fernando2[] = {
    {
    .iv = 10,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 10,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 10,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_Fernando3[] = {
    {
    .iv = 20,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ELECTRIKE,
    },
    {
    .iv = 20,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 20,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_Fernando4[] = {
    {
    .iv = 30,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 30,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 30,
    .lvl = 39,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOUDRED,
    }
};

static const struct TrainerMon sParty_Fernando5[] = {
    {
    .iv = 40,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 40,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MANECTRIC,
    },
    {
    .iv = 40,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_EXPLOUD,
    }
};

static const struct TrainerMon sParty_Sawyer2[] = {
    {
    .iv = 10,
    .lvl = 6,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_SENTRET,
    },
    {
    .iv = 10,
    .lvl = 6,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_PIDGEY,
    }
};

static const struct TrainerMon sParty_Sawyer3[] = {
    {
    .nickname = _("Foggy"),
    .iv = 20,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WOOLOO,
    .moves = {MOVE_TACKLE, MOVE_GROWL, MOVE_DEFENSE_CURL, MOVE_THUNDER_WAVE},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_IMPISH,
    .ball = ITEM_LUXURY_BALL,
    },
    {
    .nickname = _("Nelson"),
    .iv = 20,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SNUBBULL,
    .moves = {MOVE_ICE_FANG, MOVE_FIRE_FANG, MOVE_THUNDER_FANG, MOVE_CHARM},
    .gender = TRAINER_MON_MALE,
    .nature = NATURE_ADAMANT,
    .ball = ITEM_LUXURY_BALL,
    }
};

static const struct TrainerMon sParty_Sawyer4[] = {
    {
    .nickname = _("Ledybaby"),
    .iv = 30,
    .lvl = 10,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LEDYBA,
    .moves = {MOVE_SUPERSONIC, MOVE_SWIFT, MOVE_HIDDEN_POWER, MOVE_BRICK_BREAK},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_DREAM_BALL,
    },
    {
    .nickname = _("Ignesect"),
    .iv = 30,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LARVESTA,
    .moves = {MOVE_FLAME_CHARGE, MOVE_STRING_SHOT, MOVE_WILL_O_WISP, MOVE_PROTECT},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_NEST_BALL,
    },
    {
    .nickname = _("Fly-Bye"),
    .iv = 30,
    .lvl = 12,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_CUTIEFLY,
    .moves = {MOVE_STUN_SPORE, MOVE_DRAINING_KISS, MOVE_WONDER_ROOM, MOVE_LEECH_LIFE},
    .nature = NATURE_MODEST,
    .ball = ITEM_LOVE_BALL,
    }
};

static const struct TrainerMon sParty_Sawyer5[] = {
    {
    .nickname = _("Ninja Ball"),
    .iv = 40,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_HIDDEN,
    .species = SPECIES_KOFFING,
    .moves = {MOVE_POISON_GAS, MOVE_SMOKESCREEN, MOVE_PROTECT, MOVE_THIEF},
    .nature = NATURE_JOLLY,
    .ball = ITEM_TIMER_BALL,
    },
    {
    .nickname = _("Ninja Seed"),
    .iv = 255,
    .lvl = 12,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BUDEW,
    .moves = {MOVE_LEECH_SEED, MOVE_PROTECT, MOVE_STUN_SPORE, MOVE_SPIKES},
    .nature = NATURE_TIMID,
    .ball = ITEM_FAST_BALL,
    },
    {
    .nickname = _("Ninja Frog"),
    .iv = 40,
    .lvl = 11,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_2,
    .species = SPECIES_CROAGUNK,
    .moves = {MOVE_POISON_STING, MOVE_MUD_SLAP, MOVE_THIEF, MOVE_ROCK_TOMB},
    .nature = NATURE_ADAMANT,
    .ball = ITEM_GREAT_BALL,
    }
};

static const struct TrainerMon sParty_Gabrielle2[] = {
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    },
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_ZIGZAGOON,
    },
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOTAD,
    },
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEEDOT,
    },
    {
    .iv = 10,
    .lvl = 31,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TAILLOW,
    }
};

static const struct TrainerMon sParty_Gabrielle3[] = {
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SKITTY,
    },
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUZLEAF,
    },
    {
    .iv = 20,
    .lvl = 33,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_TAILLOW,
    }
};

static const struct TrainerMon sParty_Gabrielle4[] = {
    {
    .iv = 30,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 30,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 30,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 30,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LOMBRE,
    },
    {
    .iv = 30,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_NUZLEAF,
    },
    {
    .iv = 30,
    .lvl = 35,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMon sParty_Gabrielle5[] = {
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_DELCATTY,
    },
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_MIGHTYENA,
    },
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LINOONE,
    },
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUDICOLO,
    },
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SHIFTRY,
    },
    {
    .iv = 40,
    .lvl = 37,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SWELLOW,
    }
};

static const struct TrainerMon sParty_Thalia2[] = {
    {
    .iv = 10,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 10,
    .lvl = 34,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_HORSEA,
    }
};

static const struct TrainerMon sParty_Thalia3[] = {
    {
    .iv = 20,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 20,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 20,
    .lvl = 36,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMon sParty_Thalia4[] = {
    {
    .iv = 30,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 30,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILMER,
    },
    {
    .iv = 30,
    .lvl = 38,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_SEADRA,
    }
};

static const struct TrainerMon sParty_Thalia5[] = {
    {
    .iv = 40,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_LUVDISC,
    },
    {
    .iv = 40,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WAILORD,
    },
    {
    .iv = 40,
    .lvl = 40,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KINGDRA,
    }
};

static const struct TrainerMon sParty_Mariela[] = {
    {
    .iv = 0,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CHIMECHO,
    }
};

static const struct TrainerMon sParty_Alvaro[] = {
    {
    .iv = 0,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BANETTE,
    },
    {
    .iv = 0,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KADABRA,
    }
};

static const struct TrainerMon sParty_Everett[] = {
    {
    .iv = 0,
    .lvl = 41,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_WOBBUFFET,
    }
};

static const struct TrainerMon sParty_Red[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_CHARMANDER,
    }
};

static const struct TrainerMon sParty_Leaf[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_BULBASAUR,
    }
};

static const struct TrainerMon sParty_BrendanLinkPlaceholder[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_GROUDON,
    }
};

static const struct TrainerMon sParty_MayLinkPlaceholder[] = {
    {
    .iv = 0,
    .lvl = 5,
    .evs = {0, 0, 0, 0, 0, 0},
    .ability = ABILITY_SLOT_1,
    .species = SPECIES_KYOGRE,
    }
};
